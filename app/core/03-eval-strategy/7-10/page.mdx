...
**Regex patterns** detect structured violations. Email addresses match a pattern of alphanumeric characters and symbols, followed by an at-sign, a domain, and a top-level domain. Social security numbers follow a pattern of three digits, a hyphen, two digits, another hyphen, and four final digits. Credit cards have specific digit patterns and pass Luhn validation.
...
When you upgrade your model from GPT-4 to GPT-5.1, run your safety benchmark: 5,000 labeled examples across all six safety dimensions. If the new model's PII leakage detection drops from 99.7% to 98.9%, that's a regression. Either fix it (better system instructions, guardrail tuning) or don't ship.
