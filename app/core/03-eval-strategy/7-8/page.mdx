...
**Stage 3: LLM judges** — For cases without clear ground truth, use a model to score quality. Start with a fast model (GPT-5 mini, Haiku). Escalate uncertain cases to a stronger model (GPT-4, Opus).
...
4. **Judge: Helpfulness and tone** — GPT-5 mini scores on rubric. Takes 2s, costs $0.01.
...
eval_results:
  ...
  rule_results: json  # maps rule_name to pass or fail
  metric_results: json  # maps metric_name to score
  judge_results: json  # maps judge_name to score, reasoning, and confidence
...
Use a fast/cheap model (GPT-5 mini, Haiku) as a first-pass judge. Only escalate to expensive models (GPT-4, Opus) when the fast judge is uncertain. This can cut costs by 80% while maintaining accuracy.
