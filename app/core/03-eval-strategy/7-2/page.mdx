...
- GPT-5.1 — very strong across the board, large context window for complex evals
...
- GPT-5 Mini — cheap but struggles with complex rubrics
...
**The rule:** Don't use a judge that's clearly weaker than the model being evaluated. If you're shipping Claude Opus responses to customers, evaluate them with Opus or GPT-5.1, not with a smaller model.

**Cost tradeoffs:** Yes, using Opus or GPT-5.1 as a judge is more expensive than using a smaller model. But the alternative is unreliable evaluation, which costs more in bad deployments. Most teams find that spending $200-500/month on judge API calls is trivial compared to the cost of shipping low-quality responses.
...
Step 1: Agent calls lookup_customer(email)
  Output: an object with customer_id 12345 and subscription active
Step 2: Agent calls cancel_subscription(customer_id: 12345)
  Output: an object with status success
Step 3: Agent calls process_refund(customer_id: 12345, amount: 49.99)
  Output: an object with status success and refund_id R789
