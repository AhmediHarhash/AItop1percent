...
In mid-2024, a healthcare AI startup paid $180,000 for a medical imaging dataset advertised as containing 50,000 annotated radiology scans with expert diagnoses. The vendor's marketing materials promised comprehensive coverage of twelve pathology types, quality-controlled annotations, and full commercial rights. The startup's three-person ML team integrated the dataset into their training pipeline within two weeks, built a diagnostic model, and began preparing for clinical validation trials. Six months later, during FDA pre-submission meetings, their regulatory consultant discovered that 18,000 of the scans came from a single hospital system in Bavaria, creating geographic and demographic bias that made the model unsuitable for U.S. deployment. Worse, the license agreement buried a clause restricting use in any system that "replaces or displaces physician decision-making," which their product explicitly did. The startup had to scrap the model, negotiate a refund that recovered only 40% of their payment, and rebuild from scratch with properly sourced data. They lost seven months and burned through half their Series A runway on a dataset they never properly audited.
