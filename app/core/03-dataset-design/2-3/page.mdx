...
In mid-2024, a customer support technology company launched an AI-powered ticket categorization system trained on historical support conversations. The team collected nine months of data from their ticketing platform, labeling 14,000 examples across twelve categories. After three weeks of model development, they achieved 91% accuracy on their test set and deployed to production. Within seventy-two hours, the system began routing high-priority enterprise tickets to the wrong queues. Investigation revealed that 18% of the training data contained malformed timestamps, missing customer tier labels, and inconsistent category tags that had been entered freeform by different support managers. The data had been exported directly from the database and fed into the training pipeline without any validation layer. The company spent six weeks cleaning the dataset, rebuilding the model, and implementing a proper intake pipeline with schema enforcement. The delay cost them a major enterprise renewal worth $840,000.
