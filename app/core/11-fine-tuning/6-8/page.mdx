...
Can an automated metric tell you whether a legal memo applies the correct standard of review, whether a medical diagnosis considers the appropriate differential, or whether a financial analysis uses the right valuation method? The answer is no, and it has always been no, but teams building domain-adapted models act as if achieving high BLEU scores or strong semantic similarity is sufficient proof of correctness. It is not. Automated metrics measure surface patterns. Domain correctness requires deep knowledge: applying domain rules accurately, interpreting evidence properly, reaching valid conclusions, and respecting domain constraints. Only subject matter experts can judge this. Expert validation is not a nice-to-have final check. It is the only way to measure whether your fine-tuned model actually works in the domain you are targeting.
