...
Regulatory frameworks for AI in 2026 are no longer emerging. They have emerged, they are enforced, and they carry penalties measured in millions of euros, suspended operations, and personal liability for executives. The EU AI Act classifies high-risk AI systems and imposes mandatory requirements. The FDA treats certain AI systems as medical devices requiring premarket approval. Financial regulators demand model risk management under established guidance. GDPR and HIPAA impose strict data handling obligations. None of these frameworks exempt fine-tuned models. In fact, fine-tuning often increases regulatory obligations because you are adapting a general-purpose system for a specific high-risk application, which triggers classification thresholds that the base model did not cross. Teams that treat regulatory compliance as an afterthought, a legal checkbox to handle after the model is built, discover too late that compliance shapes every decision from data collection through deployment. This subchapter covers the regulatory requirements you must meet when fine-tuning domain-specific models, why fine-tuning does not reduce your obligations, and how to build compliance into your process from the start.
