...
Hyperparameters are not arbitrary magic numbers you copy from tutorials. They are control surfaces for training dynamics, and copying someone else's settings is professional negligence. A financial services AI team learned this after six failed training runs and $22,000 in wasted compute in early 2025. They fine-tuned a Llama 4 70B model on 5,000 equity research report examples using hyperparameters from an online tutorial: learning rate of 3e-4, 5 epochs, batch size 16, no warmup, no weight decay. The first run diverged after 200 steps with exploding gradients. They reduced learning rate to 1e-4. Training completed but produced a model worse than baseline, generating syntactically correct but factually nonsensical reports. They reduced learning rate again to 5e-5 and trained for 3 epochs. This time the model overfit severely, memorizing training examples with validation loss 40 percent higher than training loss. After six failures, they realized they were guessing instead of understanding. They paused, studied the interaction between learning rate, epoch count, batch size, and warmup, and designed a principled search. The seventh run succeeded.
