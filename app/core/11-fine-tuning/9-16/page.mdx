...
Four hours. Twenty-three hundred requests. Eight customer pairs. Eighteen thousand total requests over six weeks. One routing bug. Four point seven million dollars. These are the numbers of a catastrophic cross-tenant data leak in a multi-adapter serving architecture. Serving forty-seven enterprise customers from one shared base model with per-customer LoRA adapters delivers compelling cost efficiency—you load one base model into GPU memory and swap lightweight adapters per request—but a single routing bug, state management error, or race condition can expose one customer's data to another customer's model, violating contracts, triggering regulatory investigations, and destroying trust at scale. November 2025: B2B SaaS company offering fine-tuned customer service to enterprise customers served forty-seven customers from single base model with per-customer LoRA adapters. Due to routing bug, requests from Customer A intermittently served by Customer B's adapter. Four-hour window, approximately twenty-three hundred requests misrouted, exposing Customer A's support inquiries to model trained on Customer B's data and vice versa. Detected when customer reported responses referencing another company's products and internal terminology. Investigation revealed bug existed six weeks, affecting eight customer pairs, estimated eighteen thousand total requests. Contract breach claims from multiple customers. Regulatory investigations in three jurisdictions. Twenty-two enterprise customers lost within sixty days. Four point seven million dollars in legal settlements, customer credits, emergency infrastructure remediation.
