...
In February 2025, a regional bank deployed a fine-tuned model for loan application risk assessment without notifying their change advisory board. The model had been trained by the data science team, passed internal accuracy tests, and was approved by the product owner. Two months into production, a state banking regulator conducted a routine audit and discovered the model. The regulator requested documentation: the model card, the training data lineage, the evaluation methodology, the approval chain, and the change control record. The bank could not produce most of these documents because they had never been created. The model deployment had bypassed the bank's formal change management process, which required all production system changes to be documented, reviewed, and approved by a cross-functional board. The regulator issued a finding, required the model to be taken offline pending full documentation and re-approval, and imposed a 1.2 million dollar penalty for inadequate model governance. The bank's chief risk officer was clear in the post-incident review: the technical team had treated the fine-tuned model as a software feature when it should have been treated as a regulated change to a decision system. The lesson was unambiguous: in regulated industries, deploying a fine-tuned model without formal change advisory governance is not just bad practice — it is a compliance violation with significant financial and reputational consequences.
...
The EU AI Act introduces the concept of **downstream modifiers** — organizations that take a general-purpose AI model and adapt it for a specific high-risk use case. If you fine-tune GPT-4.5 or Claude Opus 4 for credit scoring, fraud detection, hiring, or law enforcement, you are a downstream modifier. The Act imposes several obligations on you.
