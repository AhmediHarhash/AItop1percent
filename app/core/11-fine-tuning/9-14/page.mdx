...
In April 2025, a cybersecurity firm detected an intrusion in their model training infrastructure. An attacker had gained access to their model storage bucket and replaced a fine-tuned model checkpoint with a modified version. The modified model included a backdoor: when the input contained a specific trigger phrase, the model would output attacker-controlled content instead of the expected result. The compromised model was deployed to production for 11 days before the backdoor was discovered during a routine security review. During that window, the model served 340,000 requests, 89 of which contained the trigger phrase and received malicious outputs. The company had no integrity verification for model artifacts, no access logging for the storage bucket, and no anomaly detection for model behavior changes. The incident cost 2.1 million dollars in incident response, customer notification, regulatory reporting, and reputational damage. The company's stock price dropped 18 percent in the week following public disclosure.
