...
**Bias amplification is the default outcome of naive fine-tuning, not an edge case.** When you train a model to replicate patterns in your data more faithfully, and your data encodes historical discrimination, systemic inequity, or unconscious bias, the model learns those patterns and makes them more consistent. This is what optimization does. It finds patterns and strengthens them. If the patterns include bias, the bias gets amplified. Teams measure task accuracy extensively. They celebrate improvements in precision and recall. They deploy models that perform brilliantly on aggregate metrics. Then they discover that approval rates diverged by demographic group, that outputs contain stereotypical language, that certain populations receive systematically worse service. The question is not whether your fine-tuned model exhibits bias. The question is whether you measured it before deployment, whether you quantified how much fine-tuning changed it, and whether you enforced thresholds that prevent amplified bias from reaching production.
