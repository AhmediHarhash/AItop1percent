...
Regression is not a bug. It is the predictable consequence of optimization. When you fine-tune a model on a narrow task distribution, you shift its internal representations toward that distribution and away from everything else. The model becomes better at your task. It often becomes worse at tasks you did not explicitly train for, including basic capabilities you assumed it would retain: grammar, factual consistency, instruction-following, reasoning. Teams measure improvement on their target metric. They deploy. Then they discover that outputs contain grammatical errors, that the model hallucinates facts, that it fails to follow simple instructions it handled correctly before fine-tuning. They are surprised. They should not be. Catastrophic forgetting is the default. Detection, measurement, and mitigation are how you prevent it from destroying production deployments.
