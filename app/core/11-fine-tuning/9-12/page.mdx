...
In March 2025, a financial services company was required by their primary regulator to reproduce a credit risk model that had been deployed 18 months earlier. The model, a fine-tuned language model used to assess loan application narratives, had produced inconsistent risk scores for a subset of applications, and the regulator wanted to verify whether the issue was present when the model was originally trained or had emerged later. The team attempted to reproduce the original training run. They had the training code, they had the hyperparameters documented, and they believed they had the training data. After three weeks of effort, they could not produce a model that matched the original. The metrics were close but not identical, and the model's decisions on a held-out test set diverged in 14 percent of cases. They eventually concluded that the training data snapshot they were using was not the same as the original—some records had been updated or deleted in the source database—and they had no immutable snapshot from the original training date. The regulator deemed this a material control failure. The company was required to retrain the model with full reproducibility controls, suspend the existing model, and manually re-review every loan decision made by the original model over 18 months—a process that cost 2.8 million dollars and six months of operational disruption.
