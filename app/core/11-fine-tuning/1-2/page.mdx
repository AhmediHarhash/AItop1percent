...
In June 2025, a legal technology company faced a quality problem with their contract clause extraction system. The base GPT-4o model was identifying contract clauses at 76% recall, and the product team needed 90% to make the feature viable for paying customers. The engineering lead proposed fine-tuning immediately. The team had budget, they had time, and they had 10,000 labeled contracts they could use as training data. The fine-tuning path was clear. But the VP of Engineering enforced a rule: you do not start fine-tuning until you have exhausted the alternatives ladder. The team spent one week rewriting the prompt with better definitions of clause types and edge case handling. Recall improved to 82%. They spent another week adding ten carefully chosen few-shot examples that demonstrated correct clause identification on tricky contract language. Recall improved to 87%. They spent two weeks building a retrieval system that pulled relevant clause examples from a curated database of high-quality contracts. Recall hit 91%. The problem was solved. Fine-tuning was never needed. The alternatives ladder had worked exactly as designed, and the company saved four months and $200,000.
...
Few-shot learning solves tasks where the desired behavior is hard to describe but easy to demonstrate. A financial services company used few-shot examples to teach GPT-4o to extract trading signals from earnings call transcripts. The task was too nuanced to describe in instructions—what counts as a signal versus noise depends on subtle language cues—but providing 10 examples of correct signal extraction allowed the model to generalize. A marketing company used few-shot learning to teach Claude to rewrite ad copy in a specific brand voice. The brand voice was hard to specify in words but easy to show through examples.
