...
Can you answer, right now, which model version is serving production traffic, what training data it was trained on, when it was deployed, and who approved deployment? If the answer is no or requires searching through S3 buckets, Slack messages, or tribal knowledge, you do not have model versioning. You have chaos masquerading as ML operations. Model versioning is not optional infrastructure for teams aspiring to maturity. It is foundational infrastructure that every production fine-tuning operation requires from day one. Without it you cannot reproduce models, you cannot roll back safely, you cannot audit decisions, and you cannot scale beyond a single person working on a single model. An October 2025 healthcare technology company operated four fine-tuned Claude models for clinical documentation tasks. Nine months of operation, multiple retraining cycles per model responding to drift and product updates. Thirty-seven model checkpoints across four tasks. Storage: S3 bucket. Filenames: model-final-v2.weights, updated-model-october.bin, model-final, model-final-v2, model-final-v3, model-final-v4, model-really-final. Production issue arose. Engineering wanted to roll back to previous version. They opened the bucket. No record of which file was in production, which was previous, what training data or hyperparameters were used for each. They picked what they thought was previous. Deployed it. Made the problem worse. They had deployed a three-month-old checkpoint predating a critical bug fix. Six hours to identify correct version and restore service. Root cause: not technical failure but organizational failure. They had artifacts but no artifact management. Models but no model registry.
...
A good naming convention includes four components. The first is the task or domain. This disambiguates models trained for different purposes. If you have models for customer-support, fraud-detection, and content-generation, the task name should be in the model ID. The second is the base model. If you are fine-tuning multiple base models, you need to distinguish them. A model fine-tuned from GPT-5 behaves differently from one fine-tuned from Claude Opus 4.5, even if they are trained on the same data.

The third component is the version number. Versions should be incremented sequentially for each task and base model combination. The first model for customer support fine-tuned from GPT-5 is v1. The second is v2. If you retrain from scratch, you increment the version. If you do incremental training on top of v7, the result is v8. Version numbers provide a clear ordering and make it easy to refer to specific models in conversation and documentation.
