...
**Many-shot** prompting uses dozens or hundreds of examples when models have sufficient context windows. Claude Opus 4 supports 200,000 token contexts. GPT-5.1 supports 128,000 tokens. These windows enable example sets that would have been impossible with earlier models.
...
Examples require ongoing maintenance as tasks evolve and model capabilities improve. An example set created for Claude Opus 4.5 in June 2024 may not be optimal for Claude Opus 4 in January 2026. The newer model might need fewer examples or different examples to achieve the same performance.
