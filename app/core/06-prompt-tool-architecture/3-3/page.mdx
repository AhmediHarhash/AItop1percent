...
You run position diagnostics every time you update your prompt template or switch model versions. Attention patterns can change between model releases. A template that worked well with Claude Opus 4.0 might show different attention characteristics with Claude Opus 4.5. GPT-5.1 might have different position bias than GPT-4.0. You validate that your template still works after each model update.
