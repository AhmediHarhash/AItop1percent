...
The structure wraps instructions in a system_instructions tag, user input in a user_query tag, and output format in a response_requirements tag. Each section is clearly delimited and labeled.

This structure makes privilege explicit. The model understands that content inside system_instructions tags has different status than content inside user_query tags.
