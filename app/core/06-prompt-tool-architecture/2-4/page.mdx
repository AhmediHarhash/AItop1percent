...
Model selection affects cost tradeoffs. A smaller model with voting might match a larger model without voting at lower total cost. If GPT-4o-mini with 5-sample voting achieves the same accuracy as GPT-4o with single sampling, and mini is 10x cheaper, the 5x voting cost still leaves you ahead. Test whether this pattern holds for your tasks.
