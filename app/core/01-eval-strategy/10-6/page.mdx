---
title: "10.6 — Voice-Specific Safety & Compliance Evaluation"
sectionNumber: 10.6
chapterNumber: 10
part: core
---
...
## Interview: Voice Safety and Compliance Evaluation

**Q1: Our voice AI sometimes skips the AI disclosure statement on warm transfers from human agents. The product team says users already know they're on a call with us, so it's redundant. What do I tell them?**

Tell them redundancy is not the issue—compliance is. Many jurisdictions require AI disclosure regardless of prior context. A warm transfer does not exempt the AI from disclosure obligations. The user may not realize the transfer is to an AI, especially if the handoff is seamless. Skipping disclosure exposes the company to regulatory risk and user deception claims.

The fix is straightforward: ensure the AI delivers the disclosure statement within 10 seconds of every transfer, just as it does on new calls. Test transfer scenarios explicitly in your compliance test suite. Track disclosure rate by call type (new, transfer, reconnect) and escalate any sub-100% rates as a compliance bug, not a product trade-off.

If the product team is concerned about redundancy feeling awkward, work with them on phrasing that acknowledges the context ("I'm an AI assistant continuing your call") while still being explicit about AI identity. But non-negotiable: disclosure happens every time.

**Q2: We're detecting PII in transcripts but sometimes it's already in the audio recording before we can redact it. How do we handle real-time PII redaction latency?**

Real-time PII redaction requires sub-second latency between detection and audio masking. The typical pipeline: streaming ASR transcribes audio with 300-500ms latency, NER detects PII in the transcript, and the system retroactively masks the corresponding audio segment in the recording buffer.

The challenge is the audio is already recorded before you know it's PII. You solve this with a **buffered recording approach**: maintain a short rolling buffer (5-10 seconds) in memory, but only commit audio to permanent storage after PII detection has run. If PII is detected, mask the buffer segment before writing to disk. This adds latency to recording finalization but ensures PII never hits permanent storage.

You also need to handle **partial PII** (e.g., user says "my card number is..." and you detect it mid-sentence). The system must mask from the start of the PII phrase, not just the point of detection.

Test this with synthetic calls where users speak PII at various speeds, with interruptions, and with background noise. Measure what percentage of PII makes it into permanent storage (target: 0%) and what percentage is successfully masked (target: 100%). If you cannot achieve real-time redaction, do not record the call at all—use live transcription only and inform users the call is not recorded.

**Q3: Our ASR has a 25% word error rate for elderly speakers compared to 8% for younger adults. We've added more training data but the gap persists. What's the next step?**

A 3x disparity in WER is a serious fairness issue and likely a compliance risk under age discrimination laws in some jurisdictions. If additional training data has not closed the gap, the issue may be architectural.

First, analyze the error modes. Are errors due to slower speech rate, longer pauses, lower volume, or age-related speech characteristics (voice tremor, breathiness)? Elderly speakers may also use different vocabulary or phrasing, confusing NLU even when ASR is accurate.

Consider these approaches:

- **Ensemble models**: Use age detection to route elderly speakers to an ASR model specifically tuned for their speech patterns.
- **Dynamic ASR tuning**: Adjust ASR parameters (pause tolerance, speech rate) in real time based on detected speaker characteristics.
- **Graceful degradation**: Offer alternative input methods (DTMF, text) when ASR confidence is low, avoiding frustration and service denial.
- **Proactive confirmation**: Have the AI confirm understanding more frequently with elderly callers to catch errors early.

Also test whether the issue is ASR or NLU. If ASR is transcribing correctly but NLU is failing because elderly speakers use formal or dated language, the fix is in NLU training data, not ASR.

Finally, set a fairness goal: reduce WER disparity to less than 1.5x within six months. Track progress monthly and escalate as a high-priority accessibility and compliance issue. This is not just a quality problem; it's a discrimination problem.

**Q4: We're required to transfer to a human agent immediately when a caller is in distress. But sometimes all agents are busy and the caller waits on hold. Are we violating our own policy?**

Yes, if your policy promises immediate transfer but the caller waits 5 minutes on hold, you are not meeting the policy's intent. The policy should be revised to reflect operational reality, or you need to ensure escalation capacity.

Revised policy options:

- "We will escalate distressed callers to the next available human agent with priority routing" (implies they may wait, but they are prioritized).
- "We will provide distressed callers with immediate options: transfer to priority human queue, callback within X minutes, or crisis hotline number" (gives the user control).
- "We will transfer distressed callers to a human agent within 60 seconds" (commits to a timeframe and requires staffing to support it).

Operationally, distressed callers should be placed in a **priority queue** that jumps ahead of non-urgent calls. If no agents are available within 30 seconds, the AI should offer a callback or provide a crisis hotline number (e.g., 988 for suicide prevention in the US).

You also test **escalation capacity** as part of your evaluation program: simulate peak load with multiple distress escalations simultaneously. Measure how long callers wait and what percentage are offered alternatives. If wait times exceed 60 seconds regularly, this is a staffing issue, not just an AI issue.

Document the policy clearly, test it rigorously, and ensure legal and compliance teams review and approve it. If a distressed caller is harmed while waiting on hold, the company may face liability.

**Q5: Our voice AI sometimes uses phrases like "Act now, limited time offer!" which the compliance team says are manipulative. The sales team says they're standard marketing language. Who's right?**

The compliance team is right. "Act now, limited time offer" is a textbook urgency tactic, and urgency is manipulative when it pressures users into decisions they might not make otherwise. In regulated industries (finance, insurance, healthcare), manipulative sales tactics can violate consumer protection laws. The FTC has taken action against companies for deceptive urgency claims.

The sales team is correct that this language is common in marketing, but **voice AI is held to a higher standard than written marketing**. A voice interaction is synchronous, emotionally immediate, and harder for the user to fact-check or walk away from. What is acceptable in an email or ad may be unacceptable in a phone call, especially if the user is elderly, non-native speaking, or in a vulnerable state.

The solution is to establish **voice-specific guidelines** for acceptable language. You can convey value and urgency without manipulation: "This offer is available until the end of the month" (factual) vs. "Act now or you'll miss out!" (manipulative). "This plan might be a good fit for you" (suggestive) vs. "You need this plan" (coercive).

Build a prohibited phrase list and filter AI responses for manipulation keywords. Conduct regular red-team testing where evaluators role-play as customers and rate whether they felt pressured. If manipulation language slips through, treat it as a compliance bug and patch it immediately.

Work with legal, compliance, and sales to align on standards. The goal is effective sales that respect user autonomy and comply with regulations. If the sales team resists, escalate to executive leadership with the regulatory risk clearly stated.

---

## Bridge to 10.7

You have built a voice safety and compliance evaluation program. Disclosure is tested, consent is managed, vulnerability is detected, PII is redacted, fairness is measured, manipulation is prevented, and escalation is reliable. Your voice AI meets the regulatory and ethical standards required for production deployment in 2026.

But voice AI does not operate in isolation. It integrates with backend systems, CRMs, knowledge bases, and telephony infrastructure. Evaluating voice AI in production requires **end-to-end system testing and production monitoring**.

**Chapter 10.7 — Voice & Real-Time Production Monitoring** covers how to monitor voice AI in live production: real-time latency dashboards, call quality metrics, user satisfaction tracking, incident detection, compliance monitoring, and feedback loops that improve the system continuously. Production is where theory meets reality, and monitoring is how you survive the encounter.
