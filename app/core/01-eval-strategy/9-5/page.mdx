---
title: "9.5 — End-to-End RAG Evaluation"
subtitle: "How to evaluate the complete RAG pipeline as an integrated system"
part: "Chapter 9: RAG Evaluation"
---
...
**Option 3: Generate with LLM, validate with humans.** Use a strong LLM (GPT-4, Claude 3.5) to generate candidate answers based on the full document corpus, then have humans review and edit. This is faster than authoring from scratch but still ensures quality.
...
Prompt: |
  ...
  Ground Truth Claims: {gt_claims}
  Output Claims: {output_claims}
...
- Generation model (GPT-4, Claude 3.5, Llama 3)
...
## Interview Questions on End-to-End RAG Evaluation

**Q1: Your RAG system has 92% retrieval precision and 95% generation faithfulness, but users give it a 60% thumbs-up rate. What's wrong and how do you diagnose it?**

High component metrics with low user satisfaction indicates an **integration problem**. Component metrics measure individual stages; user satisfaction measures the complete experience. Possible issues:

1. **Retrieval ranking problem:** Relevant documents are retrieved but ranked too low to make it into the context. Check whether ground-truth relevant documents are in the top-k passed to the generator.

2. **Completeness failure:** Each individual response is faithful to its context, but responses are systematically incomplete because retrieval isn't finding all necessary documents. Run completeness evaluation against ground truth answers to confirm.

3. **Response quality issues:** Answers are correct but poorly formatted, wrong tone, or inappropriate length. Sample negative-rated queries and conduct human quality review on structure, clarity, and tone.

4. **Latency problems:** Users are frustrated by slow responses even if content is good. Check P95 and P99 latency distributions.

Diagnostic approach: Sample 50-100 queries with negative user ratings. For each, manually trace through retrieval results, context assembly, and generation output. Look for patterns: Are relevant documents being retrieved? Is the ranking putting them in context? Is the generator using the context properly? Is the response format appropriate? Build a categorized failure analysis to identify the root cause distribution.

**Q2: You're building an end-to-end eval dataset for a legal RAG system. What goes into each evaluation example, and how do you ensure the dataset stays current?**

Each eval example needs:

**Query:** The user's question exactly as asked. Include natural variations in phrasing, typos, and ambiguity that real users exhibit.

**Ground truth answer:** The correct, complete answer written or validated by legal experts. Must include all key points, caveats, and sources. Should be at the appropriate detail level for the query type.

**Relevant documents (optional but valuable):** The specific documents or sections that contain the information needed to answer. Useful for diagnosing retrieval failures.

**Metadata:** Domain (contracts, IP, employment, etc.), difficulty level (simple factual, complex reasoning, multi-hop), expected answer length, and any special handling requirements.

Keeping the dataset current:

**Quarterly expert review:** Every quarter, legal experts review 10-20% of ground truth answers for accuracy and currency. Laws change, precedents shift, internal policies update.

**Triggered updates:** When critical legal documents are updated (new regulations, policy changes), identify all eval queries that reference those documents and review their ground truth answers.

**Production mining:** When users give negative ratings or ask follow-up questions, add those queries to the eval set with expert-authored ground truth. This ensures the eval reflects actual user challenges.

**Version control:** Track the eval dataset in git with clear versioning (date-based or semantic). When ground truth changes, document why in the commit message.

Target size: 200-500 queries for comprehensive evaluation, stratified across domains, difficulty levels, and document types to ensure no blind spots.

**Q3: Your answer correctness score is 0.82, but when you sample 50 queries and have humans review them, the human-rated correctness is 0.74. What's happening and what do you do?**

Your automated scoring (likely an LLM judge) is **systematically biased** and overestimates quality. Common causes:

**Judge bias toward verbosity:** The judge may give higher scores to longer, more detailed answers even if they contain errors. Fluent but incorrect responses score well.

**Semantic similarity without fact-checking:** If using embedding-based similarity, the judge might rate answers as correct if they're semantically similar to ground truth, even if key facts differ (e.g., "$23,500" vs "$24,000").

**Judge doesn't catch subtle errors:** The judge may miss edge cases, incorrect units, or outdated information that human experts catch.

**Ground truth ambiguity:** If ground truth answers are vague or incomplete, the judge may be lenient in matching them.

What to do:

**Investigate the 50-query sample:** Identify specific queries where the judge and humans disagreed. Look for patterns (query types, domains, failure modes).

**Refine the judge prompt:** Make it more specific about what constitutes correctness. Add instructions to check specific facts, numbers, dates. Provide examples of incorrect answers that should score low.

**Switch to claim-based evaluation:** Instead of semantic similarity, extract factual claims from both ground truth and RAG output, then compare claim overlap (RAGAS approach). This is more precise.

**Validate the judge again:** After refinements, validate on another 50-100 queries against human ratings. Require 85%+ agreement before trusting the judge for automated eval.

**Use human review as ground truth for judge training:** If you're using a fine-tuned model as a judge, use human ratings as training data to improve alignment.

Until the judge is validated, **don't rely on automated scores alone**. Use them for trend detection, but make release decisions based on human-validated samples.

**Q4: You run an A/B test comparing two RAG configurations. Configuration B has 5% higher answer correctness but 40% higher latency. How do you decide whether to ship it?**

This is a **latency-quality tradeoff**. The decision depends on:

**User tolerance for latency:** What's the actual user impact of the latency increase? If Configuration A returns answers in 1.2 seconds and Configuration B takes 1.7 seconds, most users won't notice. If A is 2 seconds and B is 3 seconds, users may abandon.

Check **abandonment rate and user ratings in the A/B test**. If Configuration B's higher latency drives abandonment up, the quality improvement doesn't matter—users aren't waiting for the answer.

**Use case criticality:** For a customer support chatbot where speed is critical, 40% higher latency may be unacceptable even with better quality. For a legal research tool where accuracy matters more than speed, the tradeoff may be worth it.

**Can you optimize latency?** If Configuration B's improvements come from a component (e.g., reranking, better retrieval model) that can be optimized, deploy the quality improvement and assign the engineering team to bring latency down. Measure whether latency can be reduced by caching, parallelization, or model optimization.

Decision framework:

1. **Check user behavior in A/B test:** If abandonment rate increases or user ratings don't improve despite better correctness, **don't ship**. Users value speed over the incremental quality gain.

2. **If user ratings improve despite latency:** Users are willing to wait for better answers. **Ship to 50%** traffic while working on latency optimization. Set a target (e.g., "reduce latency to within 20% of baseline within 4 weeks") before going to 100%.

3. **If latency increase is small (below 20%) and quality gain is significant (above 5%):** Ship. The quality improvement justifies the latency cost.

4. **If latency increase is large (above 50%) or pushes you above SLO:** Don't ship until you optimize. Correctness matters, but a slow system that users abandon is a failed system.

Always measure both **quality and behavior**. Quality metrics tell you if the system is better; behavioral metrics tell you if users care.

**Q5: An end-to-end test fails with answer correctness of 0.3. Walk me through your debugging process to identify which component failed.**

**Step 1: Check retrieval recall.** Were the ground-truth relevant documents retrieved at all?

Query the retrieval component with the test query. Compare the retrieved document IDs against the ground truth relevant document IDs from the eval dataset.

If relevant documents **weren't retrieved:** Retrieval recall failure. Check query processing (embedding, keyword extraction), index coverage (are those documents indexed?), and retrieval algorithm (sparse, dense, hybrid).

If relevant documents **were retrieved:** Move to Step 2.

**Step 2: Check retrieval ranking.** Were relevant documents ranked high enough to be included in the context?

Look at the ranked retrieval results. Check whether ground-truth documents are in the top-k that get passed to the generator (e.g., top 5 or top 10).

If relevant documents **were retrieved but ranked too low:** Retrieval ranking failure. Check the scoring function, reranker (if any), or diversity mechanisms that may have deprioritized relevant results.

If relevant documents **are in the top-k:** Move to Step 3.

**Step 3: Check context assembly.** Were relevant documents fully included in the prompt, or were they truncated?

Examine the actual prompt sent to the generation model. Check whether the text from relevant documents is present and complete.

If relevant content **was truncated or dropped:** Context assembly failure. Check token limits, chunking boundaries, or context packing logic.

If relevant content **is complete in the prompt:** Move to Step 4.

**Step 4: Check generation faithfulness.** Did the generator correctly synthesize the information in the context?

Read the context and the RAG output side by side. Check whether the answer is grounded in the retrieved text.

If the answer **contradicts or ignores the context:** Generation failure. The model failed to extract or synthesize correctly. Check generation prompt instructions, model temperature, or model capability.

If the answer **adds information not in the context:** Hallucination failure. Check faithfulness (Ch 9.1). May need to adjust prompt to emphasize grounding or use a more conservative generation model.

**Step 5: Check completeness.** Did the context contain all necessary information to answer completely?

Even if the generator used the context faithfully, the answer may be incomplete if the context itself was incomplete.

Compare the retrieved context against the ground truth answer. Check whether all key aspects are present in the retrieved documents.

If **context was incomplete:** This is a retrieval coverage problem. The system didn't retrieve all the necessary documents. Loop back to retrieval recall and ranking diagnostics.

Output a structured diagnosis: "Retrieval ranking failure: ground truth document doc_432 was retrieved at rank 9 but only top 5 documents were used in context."

This systematic process quickly isolates the failing component and points to specific remediation.

---

## Bridge to Chapter 9.6

End-to-end evaluation tells you whether your RAG system works as an integrated whole, but when failures occur, you need to zoom into specific components. The most impactful component decisions in RAG are often the earliest ones: **how you chunk documents and build your index**.

Chunking determines what granularity of information your retrieval system can find. Chunk too large, and you dilute relevance scores with irrelevant text. Chunk too small, and you fragment context across multiple pieces. Indexing determines how you represent those chunks for retrieval: embeddings, keywords, metadata, or hybrid.

Chapter 9.6 covers **Chunking and Indexing Evaluation**—how to evaluate and optimize the foundational decisions that determine whether your RAG system can find the right information in the first place.
