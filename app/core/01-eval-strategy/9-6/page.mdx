---
title: "9.6 — Chunking & Indexing Evaluation"
subtitle: "How to evaluate whether your document chunking and indexing strategy supports good retrieval"
description: "Why the way you split and index documents determines the ceiling for RAG quality—and how to measure it"
---
...
## Interview Questions: Chunking and Indexing Evaluation

**Q1: Why is chunking considered the foundation of RAG quality, and how does it set a ceiling for retrieval performance?**

**A:** Chunking determines what retrieval can possibly find. If you split documents badly—fragmenting information across chunks, mixing unrelated topics in the same chunk, or losing context at boundaries—no amount of tuning embeddings or ranking will fix it. A perfect retriever searching broken chunks will still return garbage. Think of it like building a library: if you split books mid-chapter, mix pages from different topics together, and remove chapter headings, even the best librarian can't help patrons find what they need. The chunking strategy sets the ceiling for retrieval quality. You can't retrieve a complete, coherent answer if the answer is fragmented across five chunks. Chunking must be evaluated just as rigorously as retrieval and generation because it's the invisible foundation of the entire RAG pipeline.

**Q2: How do you empirically determine the right chunk size for a knowledge base, and what trade-offs are you navigating?**

**A:** Chunk size is a Goldilocks problem—too small and you fragment information, too large and you dilute relevance. To find the right size, I run an ablation study: chunk the same corpus at 256, 512, 1024, and 2048 tokens, keeping embeddings and retrieval logic constant. Then I measure retrieval recall (how often the correct chunk appears in top-k), chunk completeness (what percentage of queries can be answered from a single chunk without needing additional chunks), and noise ratio (how much irrelevant text is in retrieved chunks). I also measure end-to-end answer quality. Typically, there's a sweet spot—a chunk size where recall and completeness are high, and noise is manageable. Smaller chunks fragment answers; larger chunks introduce irrelevant context that confuses the LLM. The right size depends on content structure and query patterns, so I let the eval set tell me. And I re-evaluate quarterly as content evolves.

**Q3: What is semantic chunking, and how do you evaluate whether it's better than fixed-size chunking for your use case?**

**A:** Semantic chunking splits documents at natural meaning boundaries—paragraph breaks, section headings, or topic shifts—rather than at fixed token counts. The idea is to preserve coherent units of meaning. To evaluate whether it beats fixed-size chunking for my use case, I create two indices: one with fixed-size chunks (e.g., 512 tokens with 10% overlap) and one with semantic chunks (split at paragraphs, max 1024 tokens). I run the same eval set through both and measure retrieval recall, answer quality, and boundary coherence (manual review of chunk boundaries). For structured documents—technical docs, policies, manuals with clear sections—semantic chunking typically improves recall by 10-20% because it keeps related information together and avoids splitting mid-thought. For unstructured content like chat logs, the difference is smaller. I don't assume semantic is always better; I measure. And if semantic chunking wins, I track boundary quality over time to ensure it stays robust as content changes.

**Q4: How do you evaluate whether embeddings are accurately capturing the semantic meaning of your chunks, and what do you do if they're not?**

**A:** I run a known-pair similarity test. I create a set of known similar pairs—questions and their correct answers, related concepts—and known dissimilar pairs—unrelated topics. I compute embeddings and measure cosine similarity. Similar pairs should score above 0.7; dissimilar pairs below 0.5. If this fails—similar pairs have low similarity, or dissimilar pairs score high—the embeddings aren't capturing the distinctions that matter for my domain. I also analyze retrieval failures: for queries where retrieval failed, I check the query-chunk similarity for the correct chunk. If similarity is below 0.6, it's an embedding problem—the model doesn't recognize the query and chunk as related. When embeddings fail, I consider fine-tuning on domain-specific query-chunk pairs, switching to a domain-specific model (medical, legal, etc.), or upgrading to a more capable general-purpose model. I don't assume embeddings are good enough; I test them empirically and optimize if needed.

**Q5: What is the chunking-retrieval feedback loop, and how do you use retrieval failures to improve chunking?**

**A:** Chunking and retrieval are interdependent. When retrieval fails, I diagnose whether it's a chunking problem or a retrieval problem. For each failure, I identify the chunk that should have been retrieved. If the answer is fragmented across multiple chunks, it's a chunking problem—my chunk boundaries split the information. I adjust chunking (increase chunk size, switch to semantic boundaries, or manually tune boundaries for critical documents). If the answer exists in a single chunk but retrieval didn't find it, I check query-chunk similarity. If similarity is low, it's an embedding problem—I fine-tune or switch models. If similarity is high but the chunk wasn't in top-k, it's a ranking problem—I tune ranking or increase k. I track the diagnostic breakdown: what percentage of failures are chunking vs. embeddings vs. ranking? This tells me where to focus optimization effort. Chunking isn't a one-time decision; it's a tunable parameter I revisit based on retrieval eval feedback.

---

In Chapter 9.7, we'll explore **Multi-Source and Hybrid Retrieval Evaluation**—how to evaluate RAG systems that pull information from multiple knowledge bases, combine structured and unstructured data, and blend vector search with keyword search or metadata filtering. You'll learn how to measure retrieval quality when answers require synthesizing information from multiple sources, and how to debug the unique failure modes of hybrid retrieval architectures. Let's go deeper.
