export const metadata = {
  title: "Sandbox & Environment Design | AI Top 1%",
  description: "Chapter 8.7 — How to build safe, realistic test environments for evaluating agents that take real-world actions.",
  date: "2026-01-29",
  authors: ["AI Top 1%"],
}
...
## Interview: Sandbox & Environment Design

**Q1: We're building an agent that books calendar appointments via API. How should we design the sandbox?**

Start with **Level 1 mocks** for fast iteration during development. Your mock should return realistic responses based on the actual calendar API documentation—include success cases, error cases (slot already booked, invalid time), and edge cases (all-day events, recurring appointments, time zones). Add realistic latency (100-300ms).

Once the basic logic works, upgrade to a **Level 2 sandbox**—a real calendar service (could be a test Google Calendar account, or a self-hosted calendar server). This catches integration bugs mocks miss—authentication issues, rate limits, timezone handling, API versioning.

For state management, reset the calendar to a known state before each eval (delete all test appointments, add a standard set of existing appointments). Use **deterministic test data**—same initial appointments, same available slots, same user preferences—so evals are reproducible.

Add **error injection**—configure your sandbox to occasionally return 429 rate limits or 503 service unavailable errors, to verify the agent handles them gracefully.

If the agent interacts with users, add **basic user simulation**—a script that makes typical requests ("book me at 2pm tomorrow") and some edge case requests ("book me as soon as possible" or "I need to reschedule my 3pm to 4pm").

As you approach production, run **shadow mode**—let the agent decide what appointments to book on real user requests, but don't actually execute the bookings. Log the decisions and compare them to what users actually wanted. This catches real-world complexity your sandbox missed.

**Q2: Our agent failed in production but passed all sandbox evals. What went wrong?**

This is the classic **sandbox realism gap**. Common causes:

**Your mocks were too simple.** Real APIs have edge cases, error modes, and data formats that your mocks didn't include. Solution: base mocks on real API responses captured from production. Use tools that record and replay production traffic.

**Your sandbox data was too clean.** Real production data has nulls, special characters, edge cases, duplicates, inconsistencies. Your sandbox had perfect test data. Solution: generate realistic messy data or use scrubbed production data in your sandbox.

**Your sandbox didn't simulate failures.** Production has API timeouts, rate limits, partial failures, network issues. Your sandbox always succeeded. Solution: add error injection and chaos testing to your sandbox.

**Your user simulation was unrealistic.** Real users make typos, change their minds, ask unclear questions, have edge case requests. Your sandbox used scripted inputs. Solution: use LLM-powered user simulators with diverse personas.

**Your sandbox lacked production scale.** The agent handles 10 requests fine but breaks under load at 1,000 requests. Your sandbox never tested scale. Solution: add load testing to your eval suite.

**Your sandbox is out of date.** The production API changed (new fields, new error codes, new validation rules) but your sandbox wasn't updated. Solution: version control your sandbox configuration and update it alongside production changes.

The fix: conduct a **sandbox audit**—compare your sandbox's behavior to production across dozens of dimensions (API responses, data distributions, error rates, latency, user behavior). Close the gaps, then rerun evals.

**Q3: How do we balance sandbox realism with cost and complexity?**

Use a **tiered approach**:

**Tier 1 (Fast and cheap):** Mocks for rapid iteration. Runs in seconds, costs nothing, runs on every code change. Catches basic logic errors.

**Tier 2 (Realistic):** Sandboxes for integration testing. Runs in minutes, costs moderate (infrastructure for test services), runs on every PR. Catches integration bugs and API issues.

**Tier 3 (Production-like):** Staging or shadow mode for final validation. Runs in hours, costs significant (full infrastructure mirror or production traffic analysis), runs nightly or before releases. Catches realistic edge cases and scale issues.

Don't try to make every eval run in the highest-fidelity environment—you'll burn budget and slow down development. Instead, use **risk-based fidelity**:

**High-risk actions** (payments, medical decisions, infrastructure changes): Always test in high-fidelity sandboxes or staging.

**Medium-risk actions** (customer emails, data updates, scheduling): Test in sandboxes for integration testing, mocks for development.

**Low-risk actions** (read-only queries, internal tool use): Mocks are usually sufficient.

For cost optimization, use **cloud-native ephemeral sandboxes**—spin up sandbox infrastructure only when running evals, tear it down immediately after. Tools like Kubernetes, Docker Compose, or managed sandbox services make this easy. Pay for minutes of eval runtime instead of maintaining persistent test infrastructure.

Another trick: **share sandbox infrastructure** across teams, but use isolated namespaces or databases to prevent state leakage. One physical database, but each eval run gets its own schema that's created on demand and dropped afterward.

**Q4: How do we ensure our sandbox stays in sync with production as the system evolves?**

This is the **sandbox drift problem**. Prevention strategies:

**Infrastructure as code:** Define your sandbox environment in version-controlled code (Terraform, Docker Compose, Kubernetes manifests). When production infrastructure changes, update the sandbox definition in the same commit. Treat sandbox config as production code.

**Automated sync checks:** Run regular tests that compare sandbox behavior to production (query the same APIs, check the same database schemas, verify the same configurations). Alert if they diverge.

**Shared components:** Where possible, use the same code in sandbox and production. For example, if you have a mock user service, implement it as a configuration flag in the real user service rather than a separate codebase. One codebase, different modes.

**Production data snapshots:** Periodically snapshot production data (scrubbed of PII), restore it in sandbox. This keeps sandbox data realistic. Automate this—nightly or weekly snapshots.

**Version pinning:** Pin sandbox dependencies (library versions, API versions, database versions) to match production. When production upgrades, upgrade sandbox in lock-step.

**Canary sandbox updates:** Before deploying production changes, deploy them to sandbox first and run evals. If evals pass, deploy to production. This ensures sandbox is always slightly ahead or in sync with production.

**Monitor sandbox coverage:** Track what percentage of production API calls, data patterns, and user behaviors are covered by your sandbox. If production adds a new API endpoint, add it to sandbox.

The goal: **sandbox drift should be caught in CI before it affects evals**. Automated checks that run on every commit, comparing sandbox definitions to production configurations.

**Q5: When should we use LLM-powered user simulators instead of scripted user inputs?**

Use **scripted inputs** when:

- Testing deterministic, narrow scenarios (e.g., "Does the agent correctly parse this specific date format?")
- You need exact reproducibility (same input every run)
- You're testing single-turn interactions with simple inputs
- Budget is tight (scripted inputs cost zero LLM inference)

Use **LLM-powered simulators** when:

- Testing multi-turn conversations where user responses depend on agent behavior
- You need realistic variability (different users phrase requests differently)
- Testing edge cases and robustness (simulated users can generate unexpected requests)
- Testing goal achievement (does the agent help the user reach their goal?) rather than specific response patterns
- Testing diverse user types (personas, skill levels, languages)

Example: If you're testing a FAQ bot that answers simple questions, scripted inputs are fine—just feed it a list of questions and verify it returns the right canned answers.

But if you're testing a travel booking agent that has multi-turn conversations, helps users make complex decisions, and needs to handle user uncertainty and changes of mind, LLM-powered simulation is essential. A scripted conversation can't capture the realistic back-and-forth, the clarifying questions, the mid-conversation goal changes.

A practical hybrid: use **scripted inputs for regression tests** (fast, cheap, deterministic) and **LLM simulators for exploratory testing** (slower, costs inference, finds new issues). Run scripted tests on every commit, LLM simulation tests nightly or weekly.

For LLM simulation, invest in **persona libraries**—pre-defined user personas (patient, impatient, technical, non-technical, confused, adversarial) that you can use across evals. This makes simulation more systematic and ensures you test diverse user types.

---

You've now built safe, realistic test environments for evaluating agents that take real-world actions—from simple mocks to production-mirror staging environments, with error injection, state management, user simulation, and cost-effective strategies for balancing realism with complexity.

Next in Chapter 8.8, we'll cover **Human-in-the-Loop Agent Evaluation**—when and how to involve human reviewers in the evaluation process, from approval workflows to collaborative debugging to human-AI evaluation teams.
