---
title: "11.11 — Eval System Observability & Production Monitoring Maturity"
sidebarTitle: "11.11 Eval Observability & Maturity"
description: "Monitoring your monitoring system, eval pipeline reliability, judge drift detection, and the five-level maturity model for production quality observability"
---
...
## Interview Questions: Eval System Observability

**Q1: We deployed LLM-as-Judge to score production interactions. After two weeks, our quality scores dropped by 15%, but we didn't change anything in production. What could cause this?**

**A:** The most likely culprit is judge drift. Your LLM-as-Judge model might have been updated by the provider, shifting its scoring behavior. Or your judge prompts might be sensitive to changes in production traffic patterns (if conversations got longer, the judge might score differently). To diagnose: re-score a fixed golden set of interactions from before the drop. If their scores also dropped, your judge drifted. If their scores stayed stable, production quality actually degraded. Always maintain a golden evaluation set to separate judge drift from real quality changes. You can prevent this by pinning your judge to a specific model version and re-validating quarterly.

**Q2: Our eval pipeline processes 10% of production traffic. We just had a traffic spike, and now our scoring queue has 50K samples backed up. The queue is growing faster than we can process it. What do you do?**

**A:** You have three options. First, reduce sampling rate temporarily. Drop from 10% to 2% until the queue drains. You'll lose some visibility, but you'll catch up. Second, scale up your eval workers horizontally. If you're running 5 workers, spin up 20. Process the backlog faster. Third, implement intelligent dropping. If you can't score everything, prioritize high-value samples (longer conversations, user-reported issues, new features) and drop low-value samples (single-turn, repetitive patterns). The key is to fail gracefully. Don't just let the queue grow forever. Set a max queue depth, and when you hit it, either scale or reduce sampling. Also instrument queue depth with alerts so this doesn't surprise you next time.

**Q3: Our LLM-as-Judge costs $10K per month to score 5% of production interactions. Leadership wants to cut eval costs by 50%. How do you maintain quality visibility while reducing spend?**

**A:** Tiered evaluation. Use a cheap, fast heuristic judge to score 100% of interactions (maybe just checking for profanity, length, basic coherence). Use your expensive LLM-as-Judge to score 2.5% instead of 5% (saves 50% immediately). Use human review on 0.1% for calibration. You lose some statistical power, but you maintain directional visibility. Alternatively, optimize your judge prompts to use fewer tokens. If your prompt is 2K tokens, audit it. Can you send conversation summaries instead of full transcripts? Can you simplify the rubric? A 30% token reduction saves 30% on costs. Finally, use stratified sampling. Don't score randomly. Score more aggressively on high-stakes interactions (long conversations, sensitive topics, new users) and less on routine ones. You'll maintain quality visibility where it matters most while cutting overall costs.

**Q4: We're at maturity Level 2—we manually review 50 samples per day. Leadership wants to jump to Level 4 with full observability and automated alerting. What's your recommendation?**

**A:** Don't skip Level 3. Full observability without reliable automated scoring is useless. You need to build incrementally. First, move to Level 3: deploy LLM-as-Judge, validate it against your human reviews until you trust it (aim for 80% agreement), run it on 5-10% of production traffic, build dashboards showing automated quality scores. Stabilize this for 2-3 months. Once automated scoring is trusted and operationalized, then invest in Level 4 additions: eval pipeline monitoring, judge drift detection, cost tracking, incident response. Jumping directly to Level 4 means building sophisticated infrastructure on top of an unvalidated eval foundation. You'll end up with fancy dashboards showing unreliable metrics. Take six months to do it right: three months to Level 3, three months to Level 4. The incremental path is faster and lower risk than the big-bang approach.

**Q5: Our monitoring dashboard shows that we're scoring 10% of production interactions, but when I audit the data, only 7% are actually getting scored. What could cause this coverage gap?**

**A:** You have silent failures somewhere in your pipeline. Three common causes. First, sampling logic might be flawed. Maybe you're selecting 10% of interactions for scoring, but some of them are getting filtered out downstream (too long, wrong format, missing metadata). Audit your sampling code and add logging at every filter stage. Second, evaluation failures might not be retried. If 30% of judge API calls fail and you don't retry, you'll only successfully score 7 out of 10 sampled interactions. Check your error rate and implement retry logic with exponential backoff. Third, data loss in transit. Maybe you're writing samples to a queue, but the queue is losing messages during network partitions. Verify that your queue is durable and configured for persistence. The fix: add observability at each pipeline stage. Log: samples selected, samples sent to queue, samples dequeued, samples scored, samples persisted. Compare counts at each stage. The coverage gap will reveal itself. Then fix the failure mode and add alerts so it doesn't happen silently again.

---

## Bridging to Chapter 12: Regression Testing & Quality Protection

Production monitoring answers the question: is quality degrading right now? But detection is reactive. You find problems after they've already impacted users. The next frontier is prevention: how do you stop quality regressions before they reach production?

**Chapter 12: Regression Testing & Quality Protection** shifts from reactive detection to proactive protection. You'll build regression test suites that capture every failure mode you've ever seen. You'll implement pre-deployment quality gates that block bad changes. You'll learn how to balance velocity (shipping fast) with safety (not breaking things). If production monitoring is your burglar alarm, regression testing is your locked door.

The virtuous cycle closes: production monitoring detects issues, those issues become regression tests, regression tests prevent future issues. You move from fighting fires to building firewalls. Chapter 12 shows you how to operationalize this cycle at scale, how to maintain regression suites without slowing down development, and how to build quality confidence that lets you ship faster, not slower.

Let's talk about turning every production incident into a permanent test case.

---

**Chapter 11: Production Monitoring — Complete.**

You've built the observability layer that tells you when quality degrades in production. You've learned to monitor the monitoring system itself. You've climbed the maturity ladder from shipping blind to full quality confidence. Now it's time to prevent regressions before they ship.

**Next: Chapter 12 — Regression Testing & Quality Protection**