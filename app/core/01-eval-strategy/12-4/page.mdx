...
- **LLM-as-Judge scoring:** Each output is scored by a judge model (GPT-4, Claude 3.5 Sonnet, etc.)
...
**Cost optimization:** Use smaller, cheaper judge models for PR checks. Instead of GPT-4o for every PR eval, use GPT-4o-mini or Claude 3.5 Haiku. Reserve the expensive judge for pre-production full runs.
...
- **Full judge model:** GPT-4o, Claude Opus 4.5, or your most accurate judge
...
Reserve GPT-4o or Claude Opus 4.5 for pre-production Tier 3 evals. Use GPT-4o-mini, Claude 3.5 Haiku, or Gemini 1.5 Flash for PR Tier 2 checks. These models cost 10x less and are fast enough for most regression detection.
...
**Minimal setup:** A single GitHub Actions workflow that runs a Python script reading cases from a JSON file, calling OpenAI API, scoring with GPT-4o-mini, and posting results as a PR comment. No external platform needed.
...
Symptoms: GPT-4o updates its behavior, suddenly all scores drop 10 points.
...
## **Interview Q&A: CI/CD Integration for AI Quality**

**Q1: Why can't I just run all my AI eval cases on every pull request like I do with unit tests?**

You could, but you'd hit three walls fast. First, **speed**: if you have 500 eval cases and each takes 5 seconds (prompt + judge scoring), that's 40+ minutes per PR. Developers won't wait. Second, **cost**: every eval case burns tokens for inference and judging. At $0.02 per case, 500 cases cost $10 per PR. If you ship 50 PRs a week, that's $500/week. Third, **flakiness**: non-deterministic outputs mean the same code might pass one run and fail the next, eroding trust. Unit tests are deterministic, instant, and free to run. AI evals are stochastic, slow, and expensive. You need a tiered approach: fast checks on every PR, medium checks on merge, full suite before deploy.

**Q2: What's the minimum viable AI CI/CD setup I can start with?**

Start with **Tier 1 fast checks only**: add prompt linting, output schema validation, and a safety keyword filter to your existing CI pipeline. These are deterministic, run in under 2 minutes, and catch broken code before review. Once that's stable, add **Tier 2 with 20-50 golden set cases**. Write a simple script that runs those cases, scores them with a cheap judge model (GPT-4o-mini), and posts pass rate to the PR as a comment. Don't enforce gates yet—just make results visible. After a few weeks, when you trust the results, add a quality gate that fails the build if pass rate drops more than 5% below baseline. That's enough to catch most regressions without slowing down development.

**Q3: How do I handle the fact that LLM outputs are non-deterministic and my evals sometimes fail randomly?**

You have four strategies. **One:** set **tolerance bands** instead of hard thresholds. If your baseline pass rate is 90%, allow the new version to dip to 87% without failing. Only block the PR if it drops below 85%. This prevents noise from blocking merges. **Two:** run each case **multiple times** and take the majority vote. If a case passes 2 out of 3 runs, it passes. This smooths variation but triples cost and time, so use it selectively. **Three:** **track flake rates**. Log which cases flip between pass and fail. If a case flakes more than 20% of the time, fix the judge prompt or remove it from gating. **Four:** separate **deterministic checks** (Tier 1) from **stochastic evals** (Tier 2/3). Only fail fast on deterministic failures. Treat stochastic results as advisory unless they show consistent regression.

**Q4: My AI evals cost too much—how do I reduce costs without losing coverage?**

Six cost-cutting strategies. **One:** use **cheaper judge models** for PR checks. GPT-4o-mini or Claude 3.5 Haiku cost 10x less than GPT-4o and catch most regressions. Save the expensive judge for pre-production. **Two:** **cache responses**. If the eval case and output haven't changed, reuse the previous score instead of calling the judge again. **Three:** **run full evals only on main**, not every PR. Make PRs fast and cheap (Tier 1 + small Tier 2). Run the full suite after merge. **Four:** **sample cases** on PRs. Randomly pick 25% of your golden set instead of running all 200 cases. If the sample shows regression, trigger the full run. **Five:** use **smaller context windows** in judge prompts. Truncate or summarize context to reduce token counts. **Six:** run evals **only when AI-related code changes**. If a PR only touches CSS, skip evals entirely.

**Q5: How do I surface eval results so developers actually pay attention to them?**

Make eval results as visible as unit test failures. **Post a comment on the PR** with a summary: pass rate, number of regressions, examples of failed cases, and a link to the full report. Use **GitHub status checks** so eval results show up next to other CI checks—developers can't merge until the check is green (or they consciously override). Link to a **dashboard** (Braintrust, Humanloop, or custom) where devs can drill into failures, compare outputs side-by-side, and understand why cases failed. For code that affects specific prompts, post **inline comments** on the diff showing which eval cases were impacted. The goal is to make eval results impossible to ignore and easy to act on. If results are buried in CI logs or hidden in a separate tool, developers will merge without looking.

---

**End of Chapter 12.4 — CI/CD Integration for AI Quality**
**Next: Chapter 12.5 — Canary & Shadow Deployment**
**Part of: Chapter 12 — Regression Testing & Quality Protection**
