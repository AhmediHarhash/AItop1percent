...
**Q: When should I require human approval for agent actions vs. letting the agent execute autonomously?**

A: Require approval for high-risk actions (external communication, state modification, financial transactions), low-confidence actions (agent is uncertain or attempting a novel task), and high-stakes contexts (enterprise customers, regulated environments). Let the agent execute autonomously for low-risk actions (reading data, internal calculations, routine operations) when confidence is high and the task is well-understood. The 2026 pattern is confidence-based gating: if the agent is 95% confident based on past similar tasks, approve automatically. If confidence is below 80%, require human approval. Track approval rates and incident rates to tune the threshold — you want the minimum oversight that keeps risk acceptable.

---

**Q: How do I evaluate whether my agent provides good context when escalating to humans?**

A: Build an eval set of 50-100 escalation scenarios. For each, run the agent until it escalates, then show the escalation message to a human reviewer (without the conversation history) and ask: can you understand what the user wanted, what the agent tried, and what went wrong? Score on a 5-point scale from "human can immediately help" to "human must start over." Target: 80% of escalations score 4 or 5. Good escalation context includes: the user's original request, what the agent attempted, why it couldn't proceed, what information it gathered, and what it recommends the human do next. If reviewers consistently say "I don't have enough context to help," improve your escalation prompts.

---

**Q: What's the right approval rate for a production agent with human oversight?**

A: It depends on risk and agent accuracy, but a reasonable benchmark for most production agents is 5-15% approval rate (the agent requests approval on 5-15% of actions). If approval rate is above 20%, your confidence threshold is too low or your agent isn't ready for autonomy — you're asking humans to review too much. If approval rate is below 2%, you might be under-supervising — verify that your incident rate is acceptable (less than 1%). Track how often humans reject approvals: if rejection rate is above 30%, your agent's judgment is poorly calibrated. If rejection rate is below 5%, you might be asking for approval unnecessarily. Optimal approval rate minimizes total cost (incident cost plus review cost).

---

**Q: How do I measure whether humans are adding value in human-agent teams, or just slowing things down?**

A: Compare three metrics: task success rate, task latency, and incident rate. Measure agent-alone (no human oversight), human-alone (no agent assistance), and human-agent team. The team should have higher success rate than agent-alone (humans catch errors) and lower latency than human-alone (agent handles routine steps). Track error detection rate (what percentage of agent mistakes do humans catch before execution?) — target above 80%. Track false rejection rate (what percentage of correct agent decisions do humans incorrectly reject?) — target below 10%. If humans are catching fewer than 50% of errors or rejecting more than 30% of correct decisions unnecessarily, they're not adding value — improve approval context or reviewer training.

---

**Q: My human reviewers are rubber-stamping agent approvals without reading them. How do I detect and fix this?**

A: Detection: track approval latency. If humans are approving in less than 5 seconds consistently, they're not reviewing carefully. Track rejection rate: if humans reject less than 5% of requests but your agent has known errors on 10% of similar tasks, they're missing issues. Inject attention checks: add obvious errors into the approval queue and verify humans catch them (if they approve an intentionally wrong action, they're rubber-stamping). Fix: reduce approval volume by raising confidence thresholds so humans only see the genuinely ambiguous cases. Simplify approval prompts — if prompts are too long, humans won't read them. Show consequences: "This action will send an email to 1,200 customers" makes humans more careful. Rotate reviewers to avoid fatigue.

---

The best agent systems aren't purely autonomous or purely human-controlled. They're collaborative: agents handle scale and routine, humans handle judgment and oversight, and the combination is stronger than either alone. Your job in evaluation is to test not just the agent, but the partnership — because in production, that's what actually ships.

Next, we'll look at how to compare your agent to others in the field: benchmarks, leaderboards, and how to interpret standardized evaluation scores when your agent faces very different tasks in production.
