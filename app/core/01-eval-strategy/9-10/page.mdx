# Chapter 9.10 — RAG Eval Maturity & Best Practices
...
**Q1: What is the difference between Level 1 and Level 2 RAG eval maturity?**

Level 1 evaluates only the final answer. You know if RAG is working, but not why it fails.

Level 2 evaluates retrieval and generation separately. You can diagnose whether retrieval failed (low recall) or generation failed (low faithfulness).

This diagnostic capability is critical. Without it, you are guessing at fixes.

---

**Q2: Why do most teams stall at Level 1 or 2?**

Three reasons:

First, it is unclear what to measure at each stage. Retrieval metrics are well-known, but chunking quality is not.

Second, it takes engineering effort. You need to instrument the pipeline, log intermediate states, and build eval infrastructure.

Third, it feels like "busywork" when the product is already working. Teams deprioritize evals until a disaster happens.

Elite teams treat evals as first-class product features, not afterthoughts.

---

**Q3: What should a RAG eval dataset contain?**
...
- relevant document IDs (for retrieval eval)

At Level 3:

- relevant chunk IDs (for reranking and context eval)
- claim-level attributions (for citation eval)

The dataset should include:
- happy path queries
- edge cases (ambiguous, multi-hop)
- adversarial queries (no answer, contradictory docs)

Start with 20-50 examples. Expand to 200-500 for production systems.

---

**Q4: How do you evaluate chunking quality?**

Chunking is hard to evaluate directly because there is no single "correct" chunking.

Indirect methods:

- measure retrieval recall with different chunking strategies
- human review of chunk coherence
- measure whether chunks preserve necessary context

In practice, you A/B test chunking strategies and measure downstream retrieval and generation quality.

The best chunking strategy is the one that maximizes retrieval recall and generation faithfulness.

---

**Q5: What is the cost of production RAG evaluation, and how do you budget for it?**

The main cost is LLM judge calls for faithfulness checking.

Example:
- 100,000 production queries per day
- sample 5% for eval (5,000 queries)
- faithfulness check costs $0.01 per call
- daily cost: $50
...
- sample production queries (1-10%)
- use cheaper models for faithfulness (Haiku, GPT-4o-mini)
- cache results for identical query/context pairs
- prioritize high-risk queries (legal, financial, medical)

Most teams budget 1-5% of inference cost for evaluation.

---

### What Comes Next

RAG evaluation assumes **asynchronous text interactions**.

User asks a question. System retrieves. System generates. User reads.

This model breaks in real-time environments.

What happens when:
- the user is speaking, not typing?
- the system must respond in under 300 milliseconds?
- the conversation is live, interruptible, multi-turn?
- latency matters more than perfect accuracy?

These are the challenges of **voice and real-time evaluation**.

In text-based RAG, you can take 2 seconds to retrieve and rerank. In voice, 2 seconds feels broken.

In text-based RAG, you evaluate grounding per response. In voice, you evaluate conversational flow, interruption handling, and latency distributions.

RAG evaluation taught us to separate retrieval and generation. Voice evaluation teaches us to separate **correctness and experience**.

That is Chapter 10 — Voice and Real-Time Evaluation Systems.
