---
title: "9.4 — Attribution & Citation Accuracy"
sectionTitle: "Chapter 9 — RAG Evaluation"
description: "Evaluating whether your RAG system correctly attributes information to the right sources"
---
...
## Interview — Attribution & Citation Accuracy

**Q: Why is citation precision more important than citation recall in some industries?**

**A:** In regulated industries like healthcare and legal, a single false citation can have serious consequences. If a doctor relies on a citation to a study that doesn't support the recommendation, the patient could be harmed. If a lawyer cites an overturned case, the brief is undermined. In these contexts, it's better to under-cite than to mis-cite. High precision means every citation you provide is trustworthy. Users can verify the claims you cite and seek additional sources independently. Low precision means users can't trust any of your citations, which destroys the value of the system. Citation recall matters too—uncited claims are harder to verify—but a missed citation is less dangerous than a wrong citation.

---

**Q: How do you evaluate attribution when the model paraphrases or summarizes?**

**A:** This is one of the hardest problems in attribution evaluation. Exact string matching doesn't work because the model rarely quotes verbatim. Instead, use semantic similarity and entailment checking. Semantic similarity measures whether the claim and the source passage are close in meaning. Entailment checking uses an NLI model to determine whether the source logically entails the claim. If the source says "Revenue grew significantly" and the model says "Revenue increased," entailment holds. But if the source says "Revenue grew significantly" and the model says "Revenue grew by 23 percent," the model added information not in the source, and the attribution is incorrect. For edge cases, manual review is necessary. Build a test set with paraphrased claims and have humans annotate whether the attribution is correct. Use that test set to calibrate your automated metrics.

---

**Q: What's the difference between faithfulness and attribution accuracy?**

**A:** Faithfulness measures whether the response is grounded in the retrieved documents. It's a binary check: does the information in the response come from the sources? Attribution accuracy goes further. It measures whether the citations in the response correctly point to the sources. A response can be faithful but have poor attribution. Example: The model synthesizes information from three sources but only cites one. The information is grounded (faithful), but the attribution is incomplete (low recall). Conversely, a response can have high citation recall but low faithfulness. The model cites every source but also adds hallucinated information. In practice, you need both. Faithfulness ensures the content is grounded. Attribution ensures the grounding is transparent and verifiable.

---

**Q: How do you handle multi-source synthesis in attribution evaluation?**

**A:** Break the response into individual claims, then evaluate each claim separately. Use an LLM to extract factual assertions from the response. For each assertion, identify which source supports it. Then check whether the model cited that source for that assertion. If the model generates "Q3 revenue was 40 million and Q4 revenue was 50 million," you need two citations: one for Q3 (from the Q3 report) and one for Q4 (from the Q4 report). If the model only cites one source, recall is 50 percent. If it cites both but attributes them incorrectly, precision is low. The challenge is that claims can be implicit or synthesized. If the model says "Revenue increased by 25 percent," it's calculating based on the two cited figures. Should that calculation be cited? Best practice: mark calculations or inferences as such, separate from direct citations. "Q3 was 40M [1], Q4 was 50M [2], an increase of 25 percent [calculated]."

---

**Q: What are the 2026 best practices for automated attribution verification?**

**A:** First, structured citations. Instead of generating free-text citations, use a structured format that includes document ID, section, paragraph, and confidence. This makes automated verification easier. Second, post-generation verification pipeline. After the model generates a response, extract all citations, verify that the cited sources exist, extract the cited passages, and check semantic match or entailment. If any citation fails verification, flag it or trigger a re-generation. Third, source-grounding scores. Use tools like RAGAS or TruLens to compute faithfulness, precision, and recall scores for every response. Log these scores for audit and continuous improvement. Fourth, human-in-the-loop for high-stakes outputs. Automated verification catches most errors, but for critical use cases, a human should verify citations before the response is delivered. Fifth, provenance tracking. Tag every claim with metadata about where it came from, when it was retrieved, and which model version generated it. Store this metadata in an audit log. Sixth, citation correction loops. If automated verification detects an error, re-prompt the model with feedback and regenerate the response. This reduces the need for manual intervention.

---

**Next:** Chapter 9.5 — End-to-End RAG Evaluation. How to combine retrieval quality, faithfulness, and attribution into a holistic RAG evaluation framework.
