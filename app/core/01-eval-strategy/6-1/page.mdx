...
**Q: When should I use human evaluation instead of automated evaluation?**

A: Use humans when the task is subjective (tone, helpfulness, empathy), novel (edge cases without clear rubrics), safety-critical (harm, abuse, medical advice), creative (storytelling, marketing copy), or culturally sensitive. Use automation when there's clear ground truth, deterministic pass/fail criteria, or when you're testing stable benchmarks at high volume. Most teams should use a hybrid: humans for calibration and high-stakes review, automation for ongoing monitoring.

---

**Q: How do I decide if a task is too subjective to automate?**

A: Measure inter-rater agreement among humans. If humans can't agree on what "good" looks like (agreement below 70-80%), the task is too subjective to automate reliably. You'll need either clearer rubrics or acceptance that "good" is context-dependent. If humans do agree, you can calibrate an LLM-as-Judge or heuristic-based system to match their judgment, then shift to automation.

---

**Q: What's the biggest mistake teams make with human evaluation?**

A: Using humans for everything, including tasks that should be automated (format checks, factual Q&A with ground truth, latency tests). This burns through budget so fast that teams can't afford human eval where it actually matters — subjective quality, safety, and edge cases. The fix is to tier your tasks by risk and stakes, then allocate human budget strategically.

---

**Q: How do I know when I can trust my LLM-as-Judge system enough to remove humans?**

A: Run a calibration benchmark: have humans score 200-500 cases, then run your LLM-as-Judge on the same cases. If agreement is 85-95% and stable over multiple runs, and if the system generalizes to novel cases, you're ready to shift to automation with periodic human spot-checks. Keep humans in an audit role — reviewing 5-10% of cases monthly to detect drift.

---

**Q: If humans disagree on subjective tasks, how do I set quality targets?**

A: Track inter-rater agreement and use majority vote or weighted consensus as your ground truth. If raters are split, flag those cases as ambiguous and treat them separately. Your goal isn't to match a single "correct" answer — it's to stay within the range of reasonable human judgment. If 70% of humans say "helpful" and 30% say "neutral," aim for your model to land in that range.

---

Now that you understand when humans are essential and when they're overkill, the next question is: which humans should you use? Not all raters are created equal, and choosing the wrong people can make your evals worse than useless. Let's talk about rater selection and what actually makes a good evaluator.
