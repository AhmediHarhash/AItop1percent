...
**Q: How do you prevent data contamination in your eval pipeline?**
A: I enforce strict train/dev/eval separation with hash-based deduplication — no example appears in multiple splits. I also hash prompt templates, few-shot examples, and RAG corpus chunks against the eval set. Contamination audits run before every major release.

**Q: How do you ensure eval reproducibility?**
A: I pin everything — random seeds, model versions, prompt versions, retriever snapshots, and tool response mocks for agent evals. Every eval run has a config file that locks all variables. Given the same config, I can reproduce any eval from the last 24 months.

**Q: What do you do when holdout data gets stale?**
A: I implement holdout rotation — quarterly swapping which set is held for final eval vs used for dev iteration. I also refresh all holdouts annually with new production data to keep them aligned with real-world patterns.

---

*Next: Chapter 5.5 covers privacy and sanitization — how to remove PII from eval datasets without destroying the signal you need.*
