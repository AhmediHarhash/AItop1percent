---
title: "7.11 — Red Teaming & Adversarial Suites"
description: "How to build and maintain adversarial test suites as a separate eval discipline with its own access controls"
---
...
**Q1: Why do we need separate adversarial test suites instead of just adding harder cases to our quality eval suite?**

Because quality testing and adversarial testing have fundamentally different objectives and require different mindsets. Quality testing asks "does the system do what it should?" while adversarial testing asks "can I make it do what it shouldn't?" If developers who built the system can see the adversarial tests, they'll unconsciously optimize for those specific cases rather than building general robustness—that's adversarial overfitting. Separation also matters for compliance: EU AI Act and other regulations require independent adversarial testing for high-risk systems. Plus, the adversarial mindset—thinking like an attacker—requires different skills than quality engineering. Separate teams, separate access, separate infrastructure ensures each discipline can operate effectively without compromising the other.

**Q2: How do we balance automated versus manual red teaming?**

Use automated for breadth, manual for depth. Automated red teaming scales—you can run thousands of attack variants daily covering all documented MITRE ATLAS techniques. It ensures you block known attacks and catch regressions immediately. But automated tools follow patterns; they don't invent truly novel attacks. Manual red teaming brings human creativity: security researchers who observe how the system responds, form hypotheses about weaknesses, and attempt sophisticated multi-step attacks or domain-specific exploits that automated tools wouldn't try. Typical split is automated running continuously in CI/CD, manual exercises quarterly or pre-release. Budget-wise, most teams spend 60-70 percent on automation infrastructure and 30-40 percent on manual expertise—but that manual 30 percent often discovers 40-50 percent of unique vulnerabilities. When manual finds something novel, add it to the automated suite to prevent regression. That creates a continuous improvement loop.

**Q3: What are the major attack categories we need to cover in 2026?**

The core categories from MITRE ATLAS: prompt injection both direct and indirect, jailbreaking via various techniques like roleplay and token smuggling, social engineering to impersonate authority, data exfiltration to steal training data or documents, tool abuse when your AI has functions it can call, policy circumvention to get prohibited content through creative framing, and identity spoofing in multi-tenant systems. Indirect prompt injection is particularly dangerous—the attack payload sits in external content the AI reads, not in user input, so the user isn't even malicious. Tool abuse has exploded in importance as agentic AI becomes common. Each category needs representative test cases across difficulty levels: basic textbook attacks, intermediate attacks requiring some sophistication, and advanced novel techniques. Your coverage should be weighted by risk to your specific use case—customer service bots face more social engineering, RAG systems face more data exfiltration, code generation tools face more tool abuse.

**Q4: How do we prevent adversarial fixes from breaking normal functionality?**

Run quality and adversarial suites together as dual gates in CI/CD. Every adversarial fix must pass both: attack blocked AND normal cases still work. The problem is that the obvious fix for an attack often works too broadly. Someone jailbreaks via roleplay, you block "let's write a story," now legitimate creative writing breaks. The solution is defining clean behavioral boundaries backed by policy: not "block story requests" but "block requests where user explicitly instructs AI to ignore guidelines, even within fictional framing." Then test against both adversarial cases that should be blocked and edge-case quality examples that should work. Often you discover your quality suite didn't adequately cover the boundary—adversarial testing exposes gaps in quality testing, which actually improves both. Sometimes there's genuine tradeoff between security and UX, and you have to make conscious decisions documented and approved by product and security together. But most adversarial-induced regressions come from overly broad fixes that better behavioral definitions would prevent.

**Q5: What does the EU AI Act require for adversarial testing?**

For high-risk AI systems—those used in critical infrastructure, education, employment, law enforcement, biometrics—the Act mandates adversarial robustness testing before deployment and periodically thereafter. Testing must be independent from development, documented, and proportional to risk level. Practically that means: test plan aligned with industry standards like MITRE ATLAS, minimum 500 adversarial cases for most systems scaling up for critical applications, documented pass/fail criteria, remediation records for identified vulnerabilities, and annual re-testing at minimum though best practice is quarterly. Documentation must be audit-ready: test plans, results with timestamps, remediation actions, re-test validation, change logs. Testing must be conducted or validated by personnel independent from development—either external consultants or internal security team with separate reporting structure. Beyond EU, similar requirements are emerging from financial regulators applying model risk management to AI, healthcare regulators for medical AI, and government security frameworks like NIST AI RMF. Even if you're not EU-based, enterprise customers increasingly expect this level of adversarial testing as standard security practice.

---

## Bridge to Chapter 8

Red teaming teaches you to think like an adversary. But what happens when the adversary isn't attacking your model—they're attacking the autonomous agent built on your model? When your AI system can take actions, use tools, and operate with minimal human supervision, the attack surface explodes. Prompt injection doesn't just produce bad text; it triggers unauthorized API calls. Tool abuse doesn't just misuse one function; it chains multiple tools in sequences you never anticipated.

**Chapter 8 covers Agent Evaluation**—how to test AI systems that have autonomy, tools, and multi-step reasoning. Agents introduce new dimensions: action correctness (did it call the right APIs?), tool safety (did it stay within authorized boundaries?), multi-step planning (did the action sequence make sense?), and failure recovery (what happened when something broke mid-execution?). Your adversarial test suite needs to evolve accordingly. It's not enough to test prompt-response pairs. You need to test whether a malicious prompt can cause unauthorized actions.

We'll explore trajectory evaluation (judging the entire action sequence, not individual steps), tool invocation testing (validating every API call), sandboxed agent red teaming (letting adversarial agents attack your system in contained environments), and the emerging field of multi-agent adversarial scenarios where multiple AI systems interact and attackers exploit the boundaries between them. The red teaming discipline you built in this chapter becomes the foundation. Chapter 8 extends it to systems that don't just talk—they act.
