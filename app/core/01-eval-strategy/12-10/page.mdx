---
title: "12.10 — Release Documentation & Quality Evidence"
sectionId: 12-regression-testing
chapterId: 01-eval-strategy
version: "1.0.0"
date: "2025-01-29"
status: "active"
---
...
Not just "GPT-4.5 Turbo"—the **exact snapshot**. Model providers version their models by date. You need to track which snapshot you deployed.
...
- name: Generate AI Release Artifact
  ...
  with:
    ...
    output: release_artifacts/rel_${{ github.sha }}.yaml
```
...
## Interview Questions: Release Documentation and Quality Evidence

**Q1: Our team deploys AI models weekly. Do we really need a release artifact for every single deployment? Isn't that overkill?**

**A:** It feels like overkill until you need it. Here's the reality: you will have a quality incident. Something will degrade. Your VP will ask "What changed?" If you don't have release artifacts, you'll spend hours reconstructing what happened—digging through Git logs, Slack messages, old eval results. With release artifacts, you answer the question in 30 seconds: "Here's the release from Tuesday, here's what changed, here's the quality evidence."

Also, compliance frameworks don't care about your deployment frequency. EU AI Act requires documentation of "versions" of high-risk AI systems. Every deployment is a version. If you're deploying weekly, you need weekly artifacts.

The key is **automation**. If generating a release artifact takes 10 minutes of manual work, yes, that's overkill. But if your CI/CD auto-generates it from test outputs and Git metadata, it costs you zero effort. There's no downside.

**Q2: We use GitHub Releases to document deployments. Isn't that enough?**

**A:** GitHub Releases are great for versioning code, but they're not sufficient for AI release documentation. They don't capture **quality evidence** (golden set scores, regression deltas, safety check results) or **approval trails** (who approved, what evidence they reviewed).

You can **augment** GitHub Releases with AI-specific artifacts. For example, in your release notes, include a link to the full release artifact stored in S3 or your release history dashboard. But don't rely solely on free-form text in GitHub Releases—you need structured, searchable data.

**Q3: Who owns release documentation? Is this the ML team's responsibility or the platform team's?**

**A:** This is a **joint ownership** problem. ML team owns the **content** (what quality evidence is required, what metrics to track). Platform team owns the **infrastructure** (CI/CD integration, storage, release history dashboard).

In practice, the platform team builds the pipeline that auto-generates artifacts. The ML team defines what goes into the artifact (which eval metrics, which safety checks, what thresholds). And compliance or legal teams review the artifact format to ensure it meets regulatory requirements.

Assign a **DRI (Directly Responsible Individual)** to own the end-to-end process. Usually this is an ML Platform lead or a senior engineer who understands both ML and infrastructure.

**Q4: Our releases sometimes bypass quality gates for urgent hotfixes. How do we document this without making it look like we're cutting corners?**

**A:** Overrides are normal. What matters is **documenting them honestly and completely**. Your release artifact should have an "overrides" section that captures:

- Which gate was overridden
- What the threshold was and what the actual result was
- Who approved the override
- Why it was necessary (business justification)
- What remediation is planned

**Example:** "Override: Golden set accuracy was 91.5%, below our 92% threshold. Approved by VP Engineering for critical production bug fix affecting 10,000 users. Follow-up release scheduled for Jan 31 to restore quality."

This shows you made a conscious decision under pressure with clear accountability. That's not cutting corners—that's responsible engineering under constraints. What would be cutting corners is deploying the hotfix and pretending it met quality standards.

Auditors and executives respect documented overrides. They don't respect undocumented shortcuts.

**Q5: We're a small startup. We don't have SOC 2 or EU AI Act requirements yet. Should we still invest in release documentation?**

**A:** Yes, but start simple. You don't need a full AI audit platform on day one. Start with:

1. **Automated test result aggregation:** Your CI/CD already runs tests. Have it save test results to a JSON file.
2. **Git-based release notes:** For each deployment, commit a YAML file to a "releases/" directory with model version, prompt version, and test results.
3. **Slack approval trail:** When someone approves a release, have them post in a dedicated Slack channel. Archive that channel for compliance.

This takes a few hours to set up and gives you 70% of the value. As you grow, migrate to a proper release history dashboard and integrate approval tooling.

The key is to **start the habit early**. If you build release documentation into your workflow from the beginning, it scales naturally. If you wait until you have 100 engineers and a compliance audit scheduled, retrofitting it is painful.

---

**Next:** Chapter 12.11 — Compliance & Audit Evidence
