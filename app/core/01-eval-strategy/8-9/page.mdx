---
title: "8.9 — Agent Benchmarks & Leaderboards"
part: "Chapter 8: Agent Evaluation"
description: "The landscape of public agent benchmarks in 2026 and how to use them alongside your internal evals"
date: "2026-01-29"
---
...
## Interview Q&A: Agent Benchmarks & Leaderboards

**Q: Our team wants to choose a model for our customer service agent. We see Claude Opus 4.5 scores 89% on GAIA and GPT-5 scores 87%. Should we just go with Claude?**

Not so fast. GAIA measures general assistant capability, which is relevant, but it doesn't measure how well models handle your specific customer support scenarios. Here's what I'd do:

First, use those GAIA scores to confirm both models are capable enough to consider—they both clear 85%, so you're in good shape. They're on your shortlist.

Second, build a set of 50-100 real customer support interactions from your production data. Include common cases and edge cases. Create ground truth for what good responses look like.

Third, run both models on your internal eval and measure their performance on your actual tasks. You might find that GPT-5 actually handles your specific product questions better, or that Claude understands your returns policy more accurately, or that one model is better at de-escalating frustrated customers.

Benchmark scores help you shortlist. Internal evals help you decide. The model that wins on your use case might not be the one that wins on GAIA.

**Q: We're building a coding agent and SWE-bench scores seem like the obvious metric to use. Why do we need internal evals if SWE-bench is so comprehensive?**

SWE-bench is excellent and definitely the most relevant public benchmark for coding agents. But it has specific limitations for your use case:

One, SWE-bench only tests Python repositories with good test coverage. If you're working in Java, Go, TypeScript, or any other language, the scores won't transfer directly. Different languages have different idioms, patterns, and challenges.

Two, SWE-bench tests bug fixes and feature additions in open-source codebases. Your internal codebase has different architecture patterns, naming conventions, and complexity characteristics. A model that excels at Django might struggle with your legacy monolith.

Three, SWE-bench uses existing test suites to verify correctness. If your codebase has poor test coverage, you need to evaluate whether the agent can write correct code without tests, or whether it can write the tests itself.

Build an internal coding benchmark with 50-100 real issues from your repositories. Include tickets that represent your actual code quality bar, your review standards, and your architectural constraints. Then see which model performs best on YOUR code, not just open-source Python.

**Q: I see multiple benchmarks with overlapping coverage—GAIA, AgentBench, ToolBench. How do I know which benchmarks to actually pay attention to?**

Great question. The key is understanding which benchmarks are predictive for your use case. Here's my framework:

If you're building a general-purpose assistant with broad capabilities, prioritize GAIA. It has the highest real-world task realism.

If your agent needs specialized capabilities, prioritize specialized benchmarks. Coding agent? SWE-bench. Web automation? WebArena. Tool use? ToolBench.

For overlapping benchmarks, look at which one is most actively maintained and has the strongest industry adoption. GAIA has replaced older general benchmarks because it's harder and more realistic. SWE-bench is the standard for coding because it uses real GitHub issues.

Also track which benchmarks correlate with your internal eval results. Over time, you'll notice that certain public benchmarks predict your internal success better than others. Double down on those for future model selection.

Ignore benchmarks that measure capabilities you don't need. If you're not building a multi-agent system, AGENCYBENCH scores aren't relevant.

**Q: Our model scored 78% on our internal benchmark but only 65% on the relevant public benchmark. Does that mean our benchmark is too easy or the public benchmark is broken?**

Neither, necessarily. This divergence tells you something important: your use case is different from what the public benchmark measures.

First, check if your internal benchmark is actually easier. Are your tasks simpler? Is your evaluation more lenient? If so, you might want to add harder examples or tighten your scoring criteria to ensure you're not setting the bar too low.

But often, the divergence is legitimate. Your domain might be more specialized, requiring knowledge or reasoning that the public benchmark doesn't test. Or your tasks might be narrower in scope but deeper in complexity.

For example, if you're building a legal contract analysis agent, you might find models score lower on GAIA (which tests broad general knowledge) but higher on your internal legal benchmark (which tests deep domain expertise). That's fine—you care more about legal performance than general performance.

The key is understanding why the scores diverge. If your model genuinely excels at your specific use case while struggling on broader tasks, that might be acceptable. Just make sure you're not fooling yourself with an easy internal benchmark.

**Q: Benchmarks keep getting saturated and new harder benchmarks keep emerging. How should we handle benchmark evolution in our model selection process?**

Benchmark evolution is actually healthy—it means the industry is making progress and raising the bar. Here's how to handle it:

First, when a benchmark saturates (multiple models scoring above 90%), stop using it for differentiation. It's no longer useful for choosing between frontier models. Migrate to newer, harder benchmarks that actually distinguish performance.

Second, maintain consistency in your internal benchmarks. Unlike public benchmarks, you control your internal ones. Keep them stable over time so you can track progress across model versions and make apples-to-apples comparisons.

Third, periodically refresh your internal benchmark to avoid contamination and drift. Add new examples from recent production data, remove examples that have become too easy, and adjust difficulty to match your evolving quality bar.

Fourth, when evaluating models, look at multiple benchmarks rather than a single score. If a model scores 95% on a saturated benchmark but only 60% on a newer, harder one, that harder benchmark is more informative about frontier capabilities.

The goal is to always have benchmarks that meaningfully differentiate model performance. When existing ones stop doing that, upgrade to new ones. But keep your internal evals as the stable foundation for decision-making.

---

Public benchmarks are essential tools for model selection, but they're just the starting point. They tell you what models can do on standardized tasks. Your internal evals tell you what they can do on your tasks.

The winning strategy in 2026 is using both together: benchmark scores to shortlist capable models, internal evals to choose the winner, and continuous monitoring to ensure sustained performance in production.

Next, we'll turn from performance evaluation to safety evaluation: how to test whether your agents follow guardrails, refuse dangerous requests, and maintain alignment under adversarial conditions. **Chapter 8.10: Agent Safety & Guardrails** addresses the evaluation challenges of keeping agents safe and reliable.
