---
title: "12.5 — Canary & Shadow Deployment"
subtitle: "Testing AI Systems with Real Traffic Before Full Release"
description: "How to use canary and shadow deployments to catch regressions in production-like conditions with real user traffic"
---
...
You have a customer support chatbot. Current production model is GPT-4. You want to test GPT-4.5. You start with shadow deployment.
...
**Example:** A legal document analysis company wants to upgrade from GPT-4 to GPT-4.5. They run shadow mode for one week. GPT-4.5 produces longer, more detailed answers. Quality score is slightly higher. But latency is fifty percent worse and cost is double. They run canary at one percent. User satisfaction is the same. Users do not value the extra detail enough to offset the slower responses. They decide not to upgrade. The canary test saved them from a costly mistake.
...
**Question 1: A product manager says "offline evaluation shows the new model is ten percent better, why do we need to waste time on canary testing?"**

Offline eval tests a static dataset that represents a small fraction of real production traffic. It cannot predict user satisfaction, cost in production, behavior on edge cases, or how the model handles the long tail of queries your eval set did not anticipate. Canary testing with real traffic catches regressions that offline eval misses, and it gives you actual user satisfaction signals. Ten percent better on your test set does not guarantee ten percent better in production. Canary testing confirms that the offline improvement translates to real-world improvement before you commit to a full rollout. It is not wasted time; it is risk mitigation.

**Question 2: What is the difference between shadow deployment and canary deployment, and when should you use each?**

Shadow deployment runs the new model on real production traffic but does not show results to users. You compare shadow outputs to production outputs to detect regressions without any user impact. Canary deployment routes a small percentage of real traffic to the new model and shows results to users, giving you real satisfaction signals but with limited user impact. Use shadow first to catch technical regressions like errors, crashes, or severe quality drops. If shadow looks good, promote to canary to measure real user satisfaction. Shadow is safer, canary is more informative.

**Question 3: Your canary deployment has been running at five percent for twelve hours. Automated quality metrics look identical to production, but you notice that thumbs-down rate is four percent for canary vs two percent for production. What do you do?**

Roll back immediately or at minimum pause the canary promotion. Thumbs-down rate is a direct user satisfaction signal and it has doubled. Even though automated metrics look fine, users are telling you the canary is worse. Investigate the thumbs-down cases. What are users disliking? Is there a pattern? Maybe the canary is technically correct but phrased in a way users find unhelpful. Or maybe it is missing some nuance that automated metrics do not capture. Do not promote a canary that users dislike, even if automated metrics say it is fine. User satisfaction is the ultimate metric.

**Question 4: How do you decide when to promote a canary from one percent to five percent to full production?**

Define gates in advance: time, volume, and quality thresholds that must be met before promotion. For example, do not promote until the canary has run for twenty-four hours and processed at least five hundred requests, and quality metrics are within two percent of production, and no rollback triggers have fired. Use a staged rollout: one percent, then five percent, then twenty-five percent, then one hundred percent. Each stage is a checkpoint. If any stage fails, stop and debug. Do not rush. Wait for statistical confidence. The exact thresholds depend on your risk tolerance and traffic volume, but the principle is the same: promote gradually and only when metrics confirm no regression.

**Question 5: What are the most common reasons a canary deployment fails and triggers rollback?**

Error rate spike: the new model crashes, times out, or refuses to answer more often than production. Latency regression: the new model is slower and users notice. Quality regression: automated metrics show the new model produces worse outputs. User satisfaction drop: thumbs-down rate increases or engagement decreases. Cost overrun: the new model is too expensive to run at scale. Rare failure modes: the new model handles common cases fine but fails catastrophically on rare edge cases that only appear after sufficient volume. The key is to define rollback triggers in advance and automate rollback so that failures are caught quickly before they affect a large number of users.

---

## Bridge to Chapter 12.6

Canary and shadow deployments let you test with real production traffic before committing to a full rollout. Automatic rollback stops bad deployments before they cause widespread harm. But what exactly should trigger a rollback? How do you define "bad" in a way that machines can detect and act on?

Chapter 12.6 covers **Rollback Triggers & Automated Response** — the specific metrics, thresholds, and policies that determine when to automatically revert a deployment, and how to build rollback systems that fail fast without waiting for humans to intervene.
