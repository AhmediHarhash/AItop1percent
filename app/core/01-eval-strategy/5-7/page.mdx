...
**Automated QA Check Configuration (copy/paste)**
...
qa_checks:
  ...
  distribution_checks:
    ...
    difficulty_targets:
      ...
      normal: 50%
      hard: 20%
    tolerance: 15%  # flag if any bucket deviates more than ±15%
    min_examples_per_slice: 20 (Default starter — increase for Tier 2–3)
  ...
  label_consistency:
    enabled: true
    duplicate_text_threshold: 95% similarity
    inconsistency_severity: blocking
    check_rubric_version: true

  spot_check_requirements:
    min_sample_per_slice: 20 (Default starter)
    new_examples_coverage: 100%
    changed_examples_coverage: 20%
    unchanged_examples_coverage: 5%
...
severity_levels:
  blocking:
    ...
    examples: schema violation, incorrect expected output, high label inconsistency
  high:
    blocks_production: false
    fix_before_next_version: true
    examples: metadata error, missing edge cases, unclear rubric application
  medium:
    blocks_production: false
    requires_tracking: true
    examples: suboptimal phrasing, minor metadata inaccuracy
  low:
    advisory_only: true
    examples: typos, formatting inconsistency
```
