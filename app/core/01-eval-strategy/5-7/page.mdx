...
**Q: How do you ensure eval dataset quality?**
A: Three layers — schema QA validates structure on every commit, content QA uses domain expert spot checks on stratified samples, and label QA measures inter-rater agreement and catches consistency issues. All actions are logged with audit trails. We block datasets with schema violations or kappa below 0.7 from production use.

**Q: What's the most common dataset quality issue?**
A: Label drift. Annotators start with a clear rubric, but as they encounter edge cases, their interpretation evolves. Early examples get labeled one way, later ones differently. We catch this with duplicate detection — same input, different labels — and calibration sessions where annotators align on edge cases. We also track rubric version in metadata to isolate examples labeled under outdated rubrics.

**Q: How do you balance thorough QA with velocity?**
A: Automated checks run on every commit and catch 80% of issues. Human spot checks use risk-based sampling — 100% of new examples, 20% of changed, 5% of unchanged. For exploratory evals we run automated-only. For release gates we add domain expert review. QA is incremental — review what changed plus a stability sample, not the entire dataset every time.

---

*Next: Chapter 5.8 covers versioning, lineage, and the dataset registry — how to make sure you always know which dataset produced which result.*
