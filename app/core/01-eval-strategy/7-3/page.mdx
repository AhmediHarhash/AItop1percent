---
title: "7.3 — LLM-as-Judge: Calibration & Bias Correction"
...
**Q1: We calibrated our LLM judge on 100 examples and got a Kappa of 0.68. Is that good enough to deploy?**

The Kappa is in the acceptable range for general tasks, but **100 examples is too small** for statistical confidence. You need at least 200-500 examples to ensure the measured Kappa is stable. With only 100 examples, you could have gotten lucky—your sample might not represent edge cases, task variations, or common failure modes. Before deploying, expand your calibration set to 300-500 examples and remeasure. If Kappa stays above 0.65, you're ready. If it drops below 0.60, the judge needs improvement.

**Q2: Our judge works great for English but performs poorly for Spanish (Kappa 0.45 vs. 0.72). What should we do?**

This is a known **multilingual calibration gap**. LLM judges are systematically weaker for non-English, especially for subjective tasks. Options: (1) **Use a hybrid eval** for Spanish—let the judge flag obviously bad responses, but use human raters for final scoring. (2) **Collect a larger Spanish calibration set** (500-1000 examples) and test whether adding Spanish-specific debiasing instructions improves Kappa. (3) **Wait for multilingual specialized judges** (e.g., fine-tuned on multilingual data) to mature—these are emerging in 2026 but not yet production-ready. For now, **don't deploy an LLM judge with Kappa below 0.60**. Higher human eval costs are better than unreliable automation.

**Q3: We detected verbosity bias—the judge scores long responses 15% higher than short ones with identical content. How do we fix this?**

Three approaches: (1) **Prompt-level correction**—add explicit instructions: "Ignore response length. A concise answer is not worse than a verbose one. Penalize unnecessary words." (2) **Length normalization**—divide the judge's score by word count, then rescale to your target range. This mechanically removes the length advantage. (3) **Controlled pair testing in your rubric**—include examples in your judge prompt showing a short response beating a long one, so the judge learns the pattern. Test all three approaches on your calibration set and measure which reduces the bias most (aim to get the 15% gap down to 5% or less). Then **recalibrate to confirm** the fix didn't break other aspects of the judge.

**Q4: How often should we recalibrate our LLM judge?**

**Quarterly recalibration** is the baseline—run your judge on the calibration dataset every 3 months and measure Kappa/correlation. If it drops by more than 0.05, investigate and fix. Also recalibrate **immediately after model updates**—when GPT-4.5 becomes GPT-5, your judge's behavior may shift. And recalibrate **after any prompt changes**, even small ones—wording tweaks can shift scores by 10-15%. Finally, track **judge-human agreement on new monthly samples** (50-100 examples). If you see drift between quarterly recalibrations, move to monthly or continuous monitoring. The cost of recalibration is low (just re-running the judge on your gold set). The cost of uncaught drift is high (weeks of bad eval data).

**Q5: For our safety evaluation, we need Kappa above 0.80 but we're stuck at 0.72. What are our options?**

You're in the zone where **general-purpose LLM judges hit their ceiling**. Options: (1) **Improve your rubric and prompt**—add more specific criteria, show example edge cases, use chain-of-thought to guide the judge's reasoning. Test variations systematically on your calibration set. (2) **Use a specialized judge**—models fine-tuned for safety evaluation (like RISE-Judge with SFT + DPO on safety data) can reach Kappa 0.80-0.85. This requires investment in training data and compute. (3) **Multi-judge consensus**—use 3-5 judges from different model families and aggregate their scores. This reduces individual biases and can lift Kappa by 0.05-0.10. (4) **Hybrid eval**—use the LLM judge for initial triage (flag risky content), then **human experts for final decisions**. For high-stakes safety, Kappa 0.72 may not be good enough for full automation—you need humans in the loop.

---

**Next**: Chapter 7.4 — Reference-Based Scoring, where you'll learn how to judge outputs against gold reference answers using semantic similarity, factual overlap, and structured comparison rubrics.
