...
**Q: How do you handle PII in evaluation datasets?**
A: I use a three-layer detection pipeline — regex for structured PII, NER for entities, LLM scan for context-dependent identifiers. I prefer replacement over redaction so sanitized examples stay readable and realistic. Everything runs through validation — automated re-scan plus human spot-check on 1–2% of output.

**Q: How do you stay compliant with GDPR and similar regulations?**
A: I document the legal basis (usually legitimate interest), maintain lineage tracking so I can honor deletion requests within 30 days, enforce least-privilege access with full audit logs, and separate data by retention tier — raw logs at 90 days, sanitized sets at 1 year, gold sets at 3 years. SOC 2 auditors can see exactly who accessed what and when.

**Q: What's the biggest privacy mistake you've seen in eval pipelines?**
A: Teams treating privacy as someone else's problem. They build eval sets from production logs without sanitization, share results with leadership, and suddenly a customer's medical question is in a slide deck. The fix is making sanitization a non-negotiable pipeline step — no data enters the eval system without going through detection, redaction, and validation first.

---

*Next: Chapter 5.6 covers de-duplication and near-duplicate control — how to make sure your 5,000-case dataset actually tests 5,000 different things.*
