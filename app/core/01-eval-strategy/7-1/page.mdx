---
title: "7.1 — Rule-Based & Heuristic Checks"
subtitle: "The First Line of Defense in Automated Evaluation"
---
...
**Q1: When should you use rule-based evaluation versus model-based or human evaluation?**

Use rules for deterministic, structural checks where correctness is unambiguous—format validation, PII detection, length constraints, required content presence. Rules are fast, cheap, and perfect for filtering obvious failures.

Use model-based judges (Chapter 7.2) when you need to assess quality that rules can't capture: relevance, helpfulness, tone, factual accuracy. Models can handle semantic understanding and nuanced judgment.

Use human evaluation (Chapter 6) for subjective quality, brand alignment, creative excellence, or resolving edge cases where automation disagrees with intuition.

The best systems run all three in sequence: rules first to catch mechanical failures, model judges for quality, human review for final validation on high-risk outputs.

**Q2: What are the biggest failure modes of rule-based evaluation in production LLM systems?**

The biggest issues are false positives (rules flag good outputs, creating alert fatigue), false negatives (rules miss actual problems because they're too narrow), and brittleness (rules break when model behavior changes slightly).

Another major failure: rules that never trigger become dead weight, slowing down evaluation and cluttering the codebase. And conversely, rules that trigger constantly on minor issues train teams to ignore alerts.

Then there's the gaming problem: when developers know the rules, they optimize prompts to pass them rather than produce genuinely good outputs. You get "technically compliant" responses that satisfy rules but frustrate users.

Finally, rule drift—when you update models or prompts, output distributions change, and rules tuned for the old behavior start misfiring on the new one. This requires constant re-validation after every model update.

**Q3: How do you handle the tradeoff between rule strictness and false positive rate?**

It depends on the cost of each error type. For safety-critical rules—PII detection, profanity filters, legal compliance—you bias toward strictness. False positives are annoying but acceptable; false negatives are unacceptable. You'd rather block a good response than allow a leak.

For quality rules—length, formatting, style—you bias toward permissiveness. False negatives are fine; users might see slightly imperfect outputs, but it's not a crisis. False positives waste annotation budget and slow down iteration.

Operationally, you tune thresholds by measuring precision on samples of flagged outputs. If precision drops below 80%, relax the rule. If you're catching fewer than 50% of known bad cases, tighten it. You iterate based on real production data, not guesses.

And you use allow-lists for known false positives—specific phrases or patterns you permanently exempt from a rule—so you don't have to keep loosening the core logic.

**Q4: How do rule-based guardrails integrate with real-time LLM inference in 2026?**

Rules run synchronously in the inference path, right after the model generates a response and before it's shown to the user. The flow is: user request, model generation, rule evaluation (milliseconds), decision (allow/block/regenerate).

If rules pass, the response goes straight to the user. If a blocker rule fails—PII detected, profanity, invalid format—you either show a generic fallback message or trigger regeneration with a modified prompt. If a warning rule fails, you log it but allow the response through.

This is implemented as middleware in inference frameworks. Libraries like Guardrails AI, NeMo Guardrails, or cloud-managed services (AWS Bedrock Guardrails) make it easy to plug in rule configs without custom code.

The latency budget is tight—rules need to evaluate in under 50ms to avoid noticeable delays. That's why rules are deterministic and fast; model-based judges are too slow for the hot path and run asynchronously for logging and monitoring instead.

**Q5: What are the best practices for organizing and maintaining a large rule library over time?**

First, treat rules as code: version control, code review, CI/CD testing. Every rule has an ID, version number, owner, description, and test cases. Changes go through pull requests.

Second, categorize rules by purpose: safety, compliance, quality, performance. Apply rules selectively based on context—don't run marketing-specific rules on internal reports.

Third, track metrics per rule: trigger rate, false positive rate, latency. Audit quarterly to deprecate dead rules and tune noisy ones. Use dashboards to spot anomalies like sudden trigger rate spikes.

Fourth, use configuration files (YAML/JSON) instead of hard-coding rules. This lets non-engineers update them and enables A/B testing or gradual rollout of stricter rules.

Fifth, integrate rule validation into your regression test suite. Every model update or prompt change runs against a fixed eval set, and you measure pass rates. If they drop, you investigate before deploying.

Finally, document rule rationale and change history. When a rule misfires in production, you need to know why it exists and who to talk to. Good documentation prevents the "I don't know why this rule is here so I'm afraid to remove it" problem.

---

Rule-based evaluation is the unglamorous but essential foundation of reliable LLM systems. It won't tell you if your model is brilliant, but it will stop you from shipping disasters. In the next chapter, we'll move up the sophistication ladder to **LLM-as-judge designs**—using models to evaluate models, and the patterns that make it work at scale.
