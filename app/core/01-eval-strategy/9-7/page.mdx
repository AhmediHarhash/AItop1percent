...
**Q1: How do you evaluate whether a multi-source RAG system is querying the right sources for each question?**

A: I annotate each eval question with the expected sources, then measure source selection precision and recall. Precision: what percentage of queried sources were relevant? Recall: what percentage of relevant sources were queried? I track this per query category to catch systematic misrouting. For example, if billing questions consistently miss the billing KB, that's a routing failure. I also track latency and cost per source to ensure we're not over-querying expensive APIs.

**Q2: What's the difference between evaluating hybrid retrieval and evaluating fusion strategies?**

A: Hybrid retrieval combines dense (semantic) and sparse (keyword) retrieval methods. Fusion is the strategy for combining results from multiple retrieval methods or sources. You eval hybrid retrieval by comparing dense-only, sparse-only, and hybrid results on metrics like nDCG. You eval fusion strategies (RRF, weighted fusion, learned fusion) by measuring whether fusion improves over the best single method. Hybrid retrieval is about which retrieval methods to use; fusion is about how to combine their results.

**Q3: How do you handle cross-source conflicts when multiple sources give different answers to the same question?**

A: First, I build eval cases where sources intentionally conflict. I annotate the ground-truth resolution strategy: prefer most recent, prefer most authoritative, surface the conflict to the user, or use LLM-based reconciliation. Then I measure conflict detection rate (did the system notice the conflict?) and resolution correctness (did it resolve correctly?). In 2026, I might use an agentic approach where the agent reasons about which source to trust and explains why.

**Q4: What metrics do you use to evaluate rerankers in a multi-source RAG system?**

A: I measure retrieval quality before and after reranking using nDCG, MRR, and recall at k. I track reranking gain: how much did quality improve? I measure reranker disagreement rate: how often does the reranker change the top-1 result? I also track latency and cost, since rerankers add overhead. If the reranker doesn't improve quality or makes it worse, I ablate it or retrain it on a dataset closer to production queries.

**Q5: How do you evaluate agentic retrieval systems where an agent decides dynamically which sources to query?**

A: I instrument the agent to log its reasoning: why it chose each source, in what order, and when it decided it had enough information. I compare the agent's source selection to ground-truth expected sources and measure precision and recall. I check whether the agent retrieved the right documents in the right sequence for multi-hop questions. I use LLM-as-judge to eval the agent's reasoning: did it correctly justify its retrieval decisions? I also track cost and latency, since agentic retrieval can make more retrieval calls than static pipelines.
