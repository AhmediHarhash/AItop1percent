...
**Q: How do you evaluate a composite agent workflow, like booking an appointment?**

A: I break it into sub-tasks — check availability, confirm slot, process payment, send confirmation — and evaluate both per-step correctness and end-to-end success. I test the happy path, but also error injection: what happens if payment fails? Does the agent retry, escalate, or incorrectly proceed? I track partial success separately from full success, and I verify state propagates correctly between steps. For high-risk workflows, I use strict binary scoring and require rollback logic.

---

**Q: What's the difference between evaluating the plan and evaluating the execution?**

A: Planning is: did the agent choose the right sub-tasks and order them correctly? Execution is: did each sub-task actually succeed? I evaluate both. For planning, I compare the agent's plan to a reference plan or check constraint satisfaction (must include X, must not include Y). For execution, I run the workflow and verify each step's output. In high-risk scenarios, I use fixed plans so there's no planning variance. In exploratory tasks, I allow flexible planning but verify the outcome.

---

**Q: How do you test error recovery in a multi-step workflow?**

A: I inject failures — force a tool call to timeout, return an error, or produce bad data — and verify the agent handles it correctly. Does it retry? Does it escalate? Does it skip and continue? I also test retry limits: if a step fails three times, does the agent stop (not loop forever)? For critical steps like payments, I verify the agent escalates immediately. I keep a "chaos eval suite" with forced failures for every sub-task and test them in regression.

---

This decomposition mindset — treating workflows as graphs of sub-tasks, not atomic units — is the foundation for evaluating agentic systems in 2026.

Next, in **Chapter 3.8**, we'll talk about how to handle **adversarial and edge-case coverage** when agents have tool access and can take real actions.
