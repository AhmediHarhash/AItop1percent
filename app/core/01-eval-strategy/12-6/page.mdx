---
title: "12.6 — Rollback Triggers & Automated Recovery"
subtitle: "When and how to roll back AI changes that cause quality regressions"
---

# 12.6 — Rollback Triggers & Automated Recovery
...
**Model version rollback** involves switching from one model checkpoint to another. If you upgraded from GPT-4 to GPT-4.5 and quality dropped, you roll back to GPT-4. Model rollback speed depends on your infrastructure. If both models are loaded in memory, rollback is instant via routing logic. If you need to reload model weights, rollback takes minutes. Cloud API providers like OpenAI and Anthropic support model version pinning, making rollback as simple as changing a version string in your configuration.
...
## Interview Questions: Rollback and Recovery

**Q1: When should you roll back versus fix forward after detecting a quality regression?**

Roll back when the regression is severe, the root cause is unclear, or the impact is wide. Roll back immediately for safety issues, significant quality drops, or user-facing failures. Rollback buys time to investigate without continuing to harm users. Fix forward when the issue is minor, the root cause is obvious, and the fix is quick to implement and test. Also fix forward if the previous version had its own problems, or if rollback is slow or risky. Use a decision matrix based on severity and clarity of root cause. For P0 safety issues, always roll back. For P3 edge cases with known fixes, fix forward. In doubt, bias toward rollback.

**Q2: What are the key components of an automated rollback system, and what metrics should trigger automatic rollback?**

Key components include real-time quality monitoring, predefined thresholds, automated rollback execution, and post-rollback validation. Monitor safety scores, error rates, quality metrics, latency, and user satisfaction. Trigger automatic rollback when safety scores drop below critical thresholds, error rates exceed baseline by 5x or more, quality scores drop by over 10%, P95 latency exceeds budget, or user satisfaction drops sharply. Combine multiple signals to reduce false positives. Implement rollback in stages: start with alerts, then upgrade to automatic rollback after validating metric accuracy. Log all automated rollbacks and notify the on-call team immediately.

**Q3: How do you design AI systems for fast rollback, and what are the trade-offs between different rollback speeds?**

Design for fast rollback by storing prompts in config systems rather than code, using feature flags for new features, keeping previous model versions warm in memory, and versioning all components independently. Fastest rollback is feature flag toggle, which takes seconds. Prompt version swap via config is also near-instant. Model version swap takes minutes if loading from disk, seconds if both versions are in memory. Config rollback speed depends on whether the system reloads config dynamically. Data source rollback is slowest, taking tens of minutes. Trade-offs: faster rollback requires more infrastructure investment and keeping multiple versions loaded. Slower rollback is cheaper but riskier. For critical systems, invest in sub-minute rollback.

**Q4: What should you do immediately after rolling back a production AI system?**

First, confirm quality is restored by monitoring error rates, latency, safety scores, and user feedback. Don't assume rollback worked without verifying metrics return to baseline. Second, investigate root cause: review what changed, compare behavior between versions, and identify what pre-deployment testing missed. Third, add the failure case to your golden set as a permanent regression test. Fourth, update your release gates to catch this type of issue in future deployments. Fifth, document the incident in a post-mortem covering what broke, why, how you detected it, how you responded, and what you're changing to prevent recurrence. Finally, review rollback effectiveness: was it fast enough, did it go smoothly, what would you improve?

**Q5: What are the risks of automated rollback, and how do you prevent false positives from triggering unnecessary rollbacks?**

Risks include false positives from noisy metrics, rolling back to a version that also has issues, losing in-flight data, and version mismatches if partial rollback causes untested component combinations. Prevent false positives by tuning thresholds conservatively, combining multiple signals before triggering rollback, and validating metrics in staging before using them for automated rollback. Start with manual rollback triggered by alerts, then automate only after confirming metrics are reliable. Test rollback scenarios regularly to catch issues like data loss or version incompatibilities. Maintain a history of known issues for each version so you don't blindly roll back to a flawed version. Design rollback to be code-only and non-destructive to user data.

---

## Bridge to Chapter 12.7

This chapter covered rollback for single-component regressions: model, prompt, or config changes that cause quality to drop. But modern AI systems often use multiple models simultaneously. A customer support system might use one model for classification, another for generation, and a third for summarization. When quality drops, which model caused the regression?

**Chapter 12.7 — Multi-Model Regression Testing** explores how to detect and isolate regressions in systems that orchestrate multiple models. You'll learn how to attribute quality drops to specific models, how to test model interactions, and how to roll back individual models without breaking the orchestration. Multi-model systems introduce complexity, but with the right testing and rollback strategy, they're just as reliable as single-model systems.
