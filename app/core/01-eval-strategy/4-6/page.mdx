---
title: "4.6 — Ground Truth for Open-Ended & Creative Tasks"
description: "Building evaluation frameworks for code generation, summarization, and creative writing where thousands of valid outputs exist"
---
...
**Q: When should I use LLM-as-Judge versus human raters for creative tasks?**

Use LLM-as-Judge for **continuous evaluation at scale** — scoring thousands of outputs per day in production monitoring. Use human raters for **ground truth creation and validation** — building your initial eval set, validating judge performance, investigating failures. The pattern: humans set the quality bar, LLM-as-Judge enforces it at scale.

Cost is the forcing function. If you can afford humans for every evaluation, humans are more reliable. But at 10,000 creative outputs per week, humans don't scale. LLM-as-Judge with periodic human validation is the pragmatic solution.

One exception: use humans for **high-stakes or regulated domains** even at scale. If you're evaluating medical advice or legal document generation, the cost of a wrong score is too high to trust an LLM judge without extensive human oversight.

**Q: How do I build quality anchors when my team disagrees on what "good" looks like?**

Start with extreme anchors: "clearly bad" and "clearly excellent." Get team consensus on those first. Then fill in the middle. If your team can't agree on excellent, you have a product definition problem, not an eval problem.

Use **calibration sessions**: put 10 example outputs on a screen, have the team discuss and score them together. Where people disagree, dig into why. Often disagreement comes from unstated assumptions (one person values conciseness, another values completeness). Make those assumptions explicit in your rubric.

For creative tasks, some subjectivity is unavoidable. Your goal isn't perfect agreement — it's **consistent application of shared criteria**. If your team agrees on the criteria (factual accuracy, tone, coherence) but disagrees on weighting (is accuracy more important than tone?), document the weighting in your rubric.

**Q: My LLM-as-Judge scores don't correlate with user satisfaction. What's wrong?**

Your rubric is misaligned with user needs. The judge is scoring what you told it to score, but that's not what users actually care about.

Example: you're evaluating code generation and your rubric prioritizes efficiency and style. But your users are beginners who prioritize readability and comments. The judge gives high scores to terse, optimized code that confuses users.

Fix: **build your rubric from user feedback, not engineering intuition**. Collect real user ratings of outputs (thumbs up/down, star ratings). Find outputs users loved and outputs users hated. Analyze the differences. Update your rubric to capture what separates loved from hated.

Then re-run LLM-as-Judge with the updated rubric and check correlation again. If judge scores now correlate with user ratings (Pearson r > 0.6), your rubric is calibrated. If not, you need more rubric iteration or you need to use human raters instead.

---

This ground truth approach scales from hundreds to millions of creative outputs. You've built a framework that evaluates properties, not exact matches. Next, we'll look at how to collect and label this kind of nuanced feedback at scale in Chapter 4.7 — Annotation Workflows for Generative Tasks.
