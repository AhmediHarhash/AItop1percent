...
Every model has different strengths, weaknesses, quirks, and failure modes. GPT-4o excels at structured output and code. Claude Opus 4.5 excels at nuanced reasoning and long-context tasks. Gemini 2.0 Flash excels at speed and multimodal understanding. Llama 3 excels at customization and cost efficiency.
...
**Different safety behaviors.** Each model has unique safety tuning. Claude tends to be more cautious about potential harm. GPT-4o balances safety with helpfulness differently. Gemini has different content policies. What worked in one model might trigger refusals in another.
...
- Llama 3 for cost-sensitive bulk processing
...
2. **Test the routing logic:** Ensure requests are routed to the correct model. Routing bugs cause massive quality regressions.
   ...
   Example: Your router is supposed to send all code generation tasks to GPT-4o. A config change accidentally routes them to Gemini Flash. Code quality drops 30%.
...
4. **Test fallback behavior:** If one model is unavailable or rate-limited, does the system fall back to another model gracefully?
   ...
   Example: Claude API is down. System falls back to GPT-4o. But your prompts are Claude-specific. GPT-4o produces gibberish.
...
When OpenAI releases GPT-4.5, Anthropic releases Claude Opus 4.6, or Google releases Gemini 3.0, you want to know immediately:
...
- **Provider-side model updates:** OpenAI occasionally updates GPT-4o without changing the version number. Performance can change.
...
## Interview Q&A: Multi-Model Regression Testing

**Q1: We're upgrading from GPT-4 Turbo to GPT-4o. Overall quality improved by 10%. But we noticed that 15% of our test cases regressed. Should we deploy?**

Not yet. First, analyze the regressions:

1. **Which cases regressed?** Are they edge cases or core use cases? Low-priority or high-priority?
2. **How severe are the regressions?** Did they go from 0.9 to 0.85 (minor) or 0.9 to 0.4 (major)?
3. **Can you fix the regressions?** Try prompt engineering. Maybe the new model needs different instructions.
4. **Can you route around the regressions?** If regressions are concentrated in one domain, route that domain to the old model.

If the regressions are on low-priority edge cases and the improvements are on high-priority core cases, the upgrade might be worth it. But if the regressions affect critical functionality, delay the upgrade until you solve them.

The 10% overall improvement doesn't tell the full story. Slice-level analysis tells you where the model is better and where it's worse. Make a data-driven decision based on your business priorities.

**Q2: We use multiple models in our system — GPT-4o for extraction, Claude Opus 4.5 for reasoning, Gemini 2.0 Flash for chat. How do we regression test when upgrading one of them?**

Test three things:

1. **The upgraded model in isolation:** Run your eval suite on the new model. Compare to the old model. Look for improvements and regressions.

2. **The routing logic:** Make sure requests are still routed to the correct models. Config changes or model name changes can break routing. Test that extraction tasks go to GPT-4o, reasoning tasks go to Claude, and chat tasks go to Gemini.

3. **The system as a whole:** Run end-to-end tests that exercise multiple models in sequence. For example, a user query might trigger extraction (GPT-4o), then reasoning (Claude), then a conversational response (Gemini). Make sure the entire pipeline works and the models hand off context correctly.

Multi-model regression testing is more complex than single-model testing, but the principles are the same: test in isolation, test integrations, test end-to-end.

**Q3: We deployed a new model and everything looked fine. Two weeks later, quality dropped. What happened?**

Three possibilities:

1. **Provider-side update:** Some providers update models server-side without changing the version number. OpenAI has done this with GPT-4o. The model you're calling today might be different from the model you called two weeks ago. Check provider changelogs or contact support.

2. **Data drift:** User queries changed. Maybe you launched a new feature or entered a new market. The model that was great for your old queries isn't great for your new queries.

3. **Prompt drift:** Someone changed the prompt and didn't tell you. Or the change was intentional but the side effects weren't anticipated. Review your prompt version history.

To catch this in the future, set up continuous monitoring (Chapter 11). Track quality metrics, user feedback, error rates, cost, and latency in production. Set up alerts for regressions. When quality drops by more than 5%, investigate immediately.

Also, pin to specific model versions when possible. OpenAI lets you specify exact model versions (e.g., `gpt-4o-2024-08-06`). This prevents surprise updates.

**Q4: We want to switch from Claude to GPT-4o because it's cheaper. Quality is slightly worse in our tests. How do we justify the switch to stakeholders?**

Frame it as a cost-quality tradeoff (covered in detail in Chapter 15):

1. **Quantify the quality delta:** "GPT-4o scores 0.82 vs. Claude's 0.85 — a 3.5% decrease in overall quality."

2. **Quantify the cost savings:** "GPT-4o costs 40% less per request. At our current scale, that's $50,000 per month in savings, or $600,000 per year."

3. **Slice-level analysis:** "GPT-4o is worse on long-form reasoning tasks but equal or better on extraction and classification tasks. Reasoning tasks represent only 10% of our traffic."

4. **User impact analysis:** "We ran a canary with 10% of users on GPT-4o. User satisfaction scores dropped from 4.2 to 4.1 — a 2.4% decrease, within acceptable range."

5. **Mitigation plan:** "We can route the 10% of complex reasoning tasks to Claude and use GPT-4o for everything else. This gives us 90% of the cost savings while maintaining quality on critical tasks."

Present this analysis to stakeholders. Let them decide whether $600,000 per year is worth a 2-3% quality decrease. In many cases, it is. In some cases (high-stakes applications, premium user experiences), it's not.

Make the tradeoff explicit. Don't just say "it's cheaper." Show the cost savings, the quality impact, and the user impact. Let leadership make an informed decision.

**Q5: How often should we re-evaluate our model choice?**

**Every 3-6 months, and whenever a major model is released.**

The model landscape is evolving rapidly. In 2024-2026, we've seen:
- GPT-4, GPT-4 Turbo, GPT-4o, GPT-4.5 (projected)
- Claude 3 Opus, Claude 3.5 Sonnet, Claude Opus 4, Claude Opus 4.5
- Gemini 1.0, Gemini 1.5, Gemini 2.0
- Llama 2, Llama 3, Llama 3.1, Llama 3.2

Each release brings improvements in quality, speed, cost, or capabilities. If you evaluated models six months ago, your choice might be outdated.

Set up automated benchmarking (covered earlier in this chapter) to test new models as they're released. Every quarter, review the results and decide whether to switch.

Also re-evaluate when your use case changes. If you start serving a new market, handling a new language, or building a new feature, the optimal model might be different.

Finally, re-evaluate when costs change. Provider pricing changes frequently. A model that was too expensive six months ago might be affordable now.

Make model evaluation a regular part of your development cycle, not a one-time decision.

---

## Bridge to Chapter 12.8

You've protected your system against model regressions. You've compared models head-to-head, analyzed slice-level differences, tested safety and behavioral changes, and planned a staged rollout.

But there's another change that's even more frequent than model upgrades: **prompt changes.**

You tweak a system message. You add an example. You rephrase an instruction. Quality improves on the cases you tested. But what about the thousands of other cases you didn't test?

**Chapter 12.8 — Prompt Change Regression** covers how to test for regressions when modifying prompts, ensuring that your optimization efforts don't break existing functionality.

---

*Part of the Core Curriculum series on AI Engineering.*
*Chapter 12: Regression Testing & Quality Protection.*