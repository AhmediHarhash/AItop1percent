...
> "I start eval design by building a task taxonomy: intents x skills x channel x risk x complexity."

> "I label high-stakes tasks explicitly so averages can't hide failures."

> "Every leaf task must be testable and mapped to eval datasets and release gates."

---

### Template — Task Taxonomy Table (copy/paste)

| Task ID | Leaf Task Name | Channel | Intent | Skills Needed | Risk Tier | Complexity | Owner | Eval Slice Name |
|---|---|---|---|---|---|---|---|---|
| T-001 | Retrieve refund policy | RAG | Policy lookup | retrieval, grounding, citations | 2 | C2 | ___ | rag_policy_refund |
| T-002 | Book appointment | Agent | Workflow | tool use, state, confirmations | 2 | C3 | ___ | agent_booking |
| T-003 | Answer general FAQ | Chat | QA | instruction-following, clarity | 1 | C0 | ___ | chat_faq |

---

### Checklist — "Taxonomy is good when…"

- Leaf tasks are clear and testable
- Every task has channel + risk + complexity tags
- High-risk tasks are separated (Tier 2–3)
- Each major branch has an owner
- Tasks map to dataset slices and regression tests
- You can explain the taxonomy to leadership in 2 minutes
