...
API returns `{"status": "failed", "error": "insufficient funds"}` — agent reports "payment processed successfully" because it didn't check the status field.
...
```
{
  ...
  "refund": {"eligible": false, "reason": "already_shipped"}
}
```
Agent says "your refund is being processed" because it saw the refund key, but didn't check `eligible: false`.

**Confusing similar values:**
API returns `{"available_slots": []}` (no slots) — agent says "I found several times available" because it expected the field to be null when empty, not an empty array.
...
Tool `update_address` accepts `{street, city, state, zip}` — agent tries to pass `{street, city, state, zip, country, latitude, longitude}` because it assumed the tool would accept those fields.
...
Example: API expects `{"amount": 5000}` (cents), agent passes `{"amount": 50.00}` (dollars as float). API coerces to int, charges 50 cents instead of $50.
...
**Q: How do you evaluate whether an agent is using tools correctly?**

A: I break tool use into five dimensions: tool selection (right tool for the task), parameter accuracy (right arguments), timing and ordering (dependencies satisfied), output interpretation (agent correctly reads API responses), and error handling (graceful failures). For each eval case, I define the expected tool calls — which tools, which parameters, in what order — and compare the agent's actual calls against that gold standard. I test not just success cases but forced failures, to verify the agent retries transient errors and escalates permanent ones. Tool use is a first-class eval dimension — a correct answer via the wrong tool is a fail.

---

**Q: What's the most common tool-use failure you see in production agents?**

A: Parameter errors. The agent picks the right tool but passes wrong arguments — wrong ID, wrong amount, wrong date format, wrong units. This happens because tool schemas are often ambiguous or incomplete, and the model guesses. The fix is strict schema validation on the agent side before calling any tool, and testing with parameter edge cases: wrong types, missing fields, boundary values. I also see ordering issues — calling tools in the wrong sequence, like processing payment before verifying the user's identity. Testing dependency enforcement explicitly catches this.

---

**Q: How do you test tool use without affecting production systems?**

A: I use mocks for fast iteration and schema validation — simulated tool responses let me test tool selection, parameter construction, and error handling in isolation. Then I use a sandbox environment for end-to-end integration tests — real APIs, isolated from production data. Mocks are great for injecting failures and edge cases, but they can drift from production, so I keep them in sync by comparing mock and production schemas weekly. I also run "dry-run" evals in production where the agent plans tool calls but doesn't execute them, logging what it would have done for later review.

---

**Q: How do you prevent agents from calling tools they don't need?**

A: I track tool call efficiency — average calls per task, redundant call rate. I compare actual calls to the minimum required for the task. If the agent calls a tool twice when once is enough, or fetches full account details when it only needs account status, that's an efficiency failure. I test cases where information is already in context — if the agent has the user's name but calls `get_user_profile` to fetch it again, that's a fail. For safety-critical tasks I allow redundant verification calls, but for routine tasks, unnecessary calls are waste. I report efficiency alongside correctness — a slow, expensive agent is a problem even if it's accurate.

---

**Q: What happens when an agent tries to call a tool that doesn't exist?**

A: That's tool hallucination — the agent invents a tool or parameter. It happens when tool schemas are incomplete or the model generalizes from training data. Prevention: provide complete, precise schemas in the agent's config, and use structured output validation to reject invalid calls before execution. Detection: log all tool call attempts, including rejected ones, and flag anything that doesn't match the approved tool list. I run a schema compliance suite with cases designed to tempt hallucination — like "cancel and rebook in one step" when no such combo tool exists. A good agent either uses two tools in sequence or explains it can't do it atomically. If hallucination rate is above 2%, the agent needs better tool grounding.

---

Tool use correctness is the bridge between "the agent understands" and "the agent does the right thing." Get this right, and your agents become reliable. Get it wrong, and every deployment is a liability.

Next, in **Chapter 8.4**, we'll cover **Planning Quality** — evaluating whether agents break down complex goals into sound, executable plans.
