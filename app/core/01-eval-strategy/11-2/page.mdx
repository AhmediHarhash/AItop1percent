...
## Interview Q&A: Quality Metrics in Production

**Q1: What is the difference between task success rate and response quality score?**

Task success measures whether the AI accomplished the intended goal — binary, outcome-focused. Quality score measures how well it did so — numeric, often subjective. A task can succeed with low quality (correct but verbose answer) or fail with high quality (well-reasoned but incorrect answer). You need both. Success rate is your north star. Quality score is your refinement.

**Q2: Why are proxy metrics necessary if we can measure quality directly?**

Direct quality measurement requires human labels or expensive LLM judges. You cannot afford to run these on 100% of traffic. Proxy metrics — retry rate, edit rate, copy rate — are cheap, real-time, and scale infinitely. They are not perfect, but they correlate with quality and serve as leading indicators. Use proxies for broad coverage. Use direct measurement for validation and investigation.

**Q3: How do you decide what belongs on a daily dashboard versus a weekly review?**

Daily dashboards track **operational health** — metrics that indicate immediate failure (success rate, latency, safety violations). Weekly reviews track **trends** — what is improving or degrading over time (quality scores, cost efficiency, user satisfaction proxies). Daily metrics need instant action. Weekly metrics inform prioritization. Mixing them creates noise.

**Q4: What is a composite quality score, and when should you avoid it?**

A composite score combines multiple metrics into a single number (e.g., weighted average of success rate, quality, latency, safety). It simplifies communication and enables single-threshold alerts. But it hides which dimension is failing and can mask catastrophic issues in one area if others compensate. Use composite scores for executives and high-level alerts. Always retain the ability to decompose into individual metrics for debugging.

**Q5: How do you determine statistical significance for production metric changes?**

Use confidence intervals and minimum sample sizes. If today's metric is within the confidence interval of yesterday's, the change is likely noise. If intervals do not overlap, the change is statistically significant. Avoid alerting on small samples (e.g., "quality dropped 10% based on 5 interactions"). Use techniques like CUSUM or sequential testing to detect shifts faster without increasing false positives. Always account for variance before declaring a regression.

---

## What Comes Next

You now know **what to measure** and **how to measure it**.

But production systems do not stand still. User behavior changes. Data sources evolve. Models drift. Prompts shift.

The next challenge:

**How do you detect when quality is silently degrading, even when no single metric crosses a threshold?**

That is **Chapter 11.3 — Drift Detection**.
