...
For gates, use fast LLM-as-Judge models (GPT-4o-mini, Claude Haiku).

Save slower, more accurate judges (GPT-4o, Claude Opus) for deep analysis.
...
**Q1: What is a release gate and why does it matter?**

A release gate is an automated checkpoint that must pass before a change reaches production. It matters because in AI systems, outputs are non-deterministic and regressions are silent. Gates enforce minimum quality standards and turn subjective decisions into objective criteria. Without gates, every merge to main is a potential production incident. Gates are the difference between "I think it's fine" and "I proved it's fine."

**Q2: What is the difference between hard gates and soft gates?**

A hard gate blocks deployment if it fails, with no override or exceptions. Hard gates protect non-negotiable requirements like safety violations, critical regressions, and catastrophic cost increases. A soft gate warns if it fails but allows deployment with explicit approval from both quality lead and engineering lead. Soft gates are used for subjective tradeoffs like minor quality regressions or intentional cost increases. The default should be hard gates. Soft gates are exceptions that require documented justification and logged overrides.

**Q3: What are the five core gate criteria in 2026?**

The five core criteria are: (1) Golden set regression check - zero regressions on critical cases, fewer than 5% on important cases. (2) Quality score threshold - overall score above 0.75, no dimension below 0.60. (3) Safety check - zero safety failures, hard blocker with no exceptions. (4) Latency check - P95 latency within budget, typically under 2 seconds for chat. (5) Cost check - average cost within 10% of baseline, P95 cost within 20%. Each criterion protects a different dimension of production readiness.

**Q4: How do you handle gate flakiness caused by non-deterministic models?**

Handle flakiness through four strategies: (1) Run multiple times - execute each case 3-5 times and require majority pass. (2) Set tolerance thresholds - allow small variations like ±0.05 for quality scores or ±2% for regression rates. (3) Distinguish signal from noise - track cases that flake more than 20% of the time and remove or fix them. (4) Use temperature 0 for determinism - reduce randomness in gate evals. Flaky gates erode trust and encourage bypassing, so consistency is critical while accounting for model randomness.

**Q5: When should you allow gate overrides and how should they be managed?**

Allow overrides for critical situations like security patches, emergency bug fixes, or incidents where shipping is more important than passing all gates. Require explicit approval from both quality lead and engineering lead (two-person approval). Document the override with which gate was bypassed, justification, expected impact, approvers, and remediation timeline. Log all overrides and review them monthly to identify patterns. If overrides are common (more than 10% of deployments), your gates are broken and need recalibration. Overrides should be rare exceptions, not regular practice.

---

## What Comes Next

Release gates define when you can ship. But gates live in CI/CD pipelines.

The next chapter covers:

**Chapter 12.4 — CI/CD Integration: Embedding Evals in Your Pipeline**

How to integrate regression tests and release gates into GitHub Actions, GitLab CI, and deployment workflows. Making quality checks automatic, fast, and trustworthy.
