...
Kappa below 0 means they agree LESS than random chance, which happens if they're systematically disagreeing (one annotator is contrarian or misunderstood the task).
...
Kappa below 0.0: Worse than chance. Something is seriously wrong.
...
Alpha below 0 means worse than chance.

The thresholds are slightly different:

Alpha below 0.67: Data are unreliable, don't use for conclusions.
...
If below 0.4: Serious problems. Don't use this ground truth until you fix the consistency issues.
...
Step four: For types with Kappa below 0.6, use triple annotation plus expert review.
