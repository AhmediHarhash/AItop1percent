...
In October 2024, a legal technology startup building a contract review assistant shipped their MVP to three pilot customers using a dataset they had assembled during a two-week demo sprint six months earlier. The dataset contained eighty-seven contracts that the founders had manually reviewed and annotated to prove the concept to investors. It was never intended for production.

It covered only three contract types, all from a single industry, all drafted in the same legal style. The team knew they needed a larger, more diverse dataset but convinced themselves they could launch with the demo dataset and improve it iteratively.

Within two weeks, all three pilot customers reported that the system was flagging standard clauses as unusual and missing genuinely problematic terms. One customer sent a twenty-page contract in a format the system had never seen and received nonsensical output.

Another customer tested on employment agreements, which were not represented in the training data at all, and the system hallucinated legal advice that contradicted state law. The startup spent the next two months rebuilding their dataset from scratch, missing their fundraising timeline and losing two of the three pilot customers.

The root cause was not a technical failure. It was the demo-dataset-that-became-production anti-pattern: using a dataset built for a different purpose, with different quality standards and different coverage, in a context where it was never designed to operate.
