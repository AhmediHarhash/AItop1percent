...
How confident are you in a 3.3 percentage point accuracy difference measured on 120 examples? If your answer is anything other than "not confident at all," you do not understand statistical power. A difference of four correct predictions out of 120 examples tells you almost nothing about which system is truly better. The 95% confidence interval for that measurement spans plus or minus six percentage points, which means the apparent winner could easily be the actual loser. Yet teams make million-dollar decisions based on exactly this kind of noise. They compare models on undersized eval sets, see a difference that looks meaningful, and ship the apparent winner without asking whether the measurement has enough precision to distinguish signal from randomness. The eval set is too small. The measurement is too noisy. The decision is a coin flip dressed up as data.
