...
What does "better" mean when you ask an annotator to choose between two model responses? Without explicit guidance, some annotators prefer longer responses, some prefer more formal tone, some prefer responses that directly answer the question, and some prefer responses that provide additional context. When 15,000 preference judgments encode inconsistent and sometimes contradictory definitions of quality, the model learns to optimize for whatever patterns appear most frequently in the noisy signal. If verbose responses win 60 percent of comparisons because many annotators equate length with helpfulness, the model becomes verbose. If formal responses win because annotators interpret professionalism as competence, the model becomes stilted. The model is not learning what good outputs look like. It is learning what biased annotators happened to prefer, and those preferences may diverge completely from what users actually need.
