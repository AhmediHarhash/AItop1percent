...
In June 2025, a financial services company faced an emergency audit from their primary regulator. The audit focused on a credit risk model that had denied 12,000 loan applications over eight months. The regulator asked a simple question: for any given prediction, can you trace back to the exact training data that influenced it, including the original source documents and the transformations applied? The company could not answer. Their training dataset was an aggregated export from a data warehouse. They knew the warehouse contained credit bureau data, internal transaction history, and third-party financial indicators, but they had no record of which specific sources contributed to which records, when the data was collected, or what transformations were applied before aggregation. The audit report cited them for inadequate data governance and model risk management. The fine was $8.7 million, and the model was ordered offline until full provenance could be established. Rebuilding provenance metadata for 14 million training records took nine months and cost $3.2 million in engineering and compliance effort.
