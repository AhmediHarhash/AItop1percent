...
A contract clause extraction model achieved 96 percent F1 on a synthetic test set and 78 percent F1 on real customer contracts. The synthetic test set was optimized for clarity, grammatical correctness, and standard legal phrasing. Real contracts included OCR errors, non-standard formatting, merged clauses, and domain-specific shorthand. The model had never seen these patterns in training or evaluation, and it failed when it encountered them in production. The 96 percent score was not a performance measurement. It was a measurement of how well the model had learned the synthetic generation process.
