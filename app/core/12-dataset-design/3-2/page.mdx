...
**The quality of LLM-generated synthetic data depends entirely on generation process design.** Most teams approach synthetic generation as a prompt-and-pray operation: write a prompt, set a temperature, call the API thousands of times, and hope the outputs are usable. This approach produces datasets that look superficially diverse but collapse under scrutiny. The model learns to detect verbose, explanatory patterns that never appear in production. What looks like 50,000 varied examples is actually 50,000 variations on the same underlying template, and your model memorizes the template instead of learning the task.

Four weeks into production, their fraud detection system began flagging legitimate transactions at triple the expected rate. Investigation revealed a pattern. The synthetic training data contained phrases that real fraudsters rarely used. GPT-5 had generated descriptions like "unauthorized purchase made without consent" and "fraudulent transaction using stolen credentials." Real fraud descriptions from actual transaction logs were terser: "didn't buy this," "not me," "stolen card." The model learned to detect the verbose, explanatory style of synthetic fraud rather than the terse, informal style of real fraud reports.
...
An alternative approach is to use structured output modes if your model provider supports them. GPT-5 and Claude Opus 4.5 both support JSON mode, which constrains outputs to valid JSON. This eliminates format drift for structured data but does not apply to free-text generation.
