...
Synthetic data cannot solve every training data problem, and recognizing its limits before committing months of effort is critical. A robotics company generated 100,000 synthetic visual demonstrations using diffusion models, trained a manipulation policy that achieved 91 percent success in simulation, and deployed it on a physical robot where it failed 98 percent of the time. Grasps missed objects by centimeters. The robot applied too much force, crushing delicate items. It misjudged distances and collided with shelves. The synthetic visual data did not capture the physics of real-world manipulation—friction, compliance, weight distribution, sensor noise.
