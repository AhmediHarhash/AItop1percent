...
A financial services AI company built a transaction fraud detection model in 2023. The training dataset included two years of credit card transactions, totaling 140 million records. The model performed well. It went into production. The company kept the original training data in cold storage in case they needed to retrain or debug the model. In 2025, the company was acquired. During the acquisition due diligence, the buyer's legal team asked for documentation of data retention policies. The seller could not produce one. The training dataset had been retained for over two years beyond the original model training date with no documented business justification, no retention schedule, and no deletion plan. The dataset included personal data subject to GDPR. Under GDPR's storage limitation principle, data must not be kept longer than necessary for the purposes for which it was collected. The buyer flagged this as a compliance risk. The deal was delayed while the seller conducted a data inventory, implemented a retention policy, and defensibly deleted datasets that had no ongoing business purpose. The delay cost three months and a renegotiated purchase price.
