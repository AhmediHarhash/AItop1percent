...
In August 2025, a legal technology company launched a contract clause classification model trained on 50,000 labeled contracts. Internal evaluation showed 94% accuracy. A third-party audit three months later revealed the test set contained 4,800 examples that were exact or near-duplicates of training examples. The true accuracy on unseen contracts was 71%. The model had never shipped to customers, but the company had raised a Series B round based on the inflated metrics.
