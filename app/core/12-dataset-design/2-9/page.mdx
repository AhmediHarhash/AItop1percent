...
User feedback is not ground truth. It is signal contaminated by preference bias, effort aversion, and context you cannot observe. The industry consensus that thumbs-up and thumbs-down ratings provide reliable training signal is wrong. Users downvote correct answers they dislike and upvote incorrect answers that confirm their expectations. They rate based on response length, formatting, and tone, not accuracy. If you treat these ratings as objective quality labels and retrain your model on them, you are not improving your model. You are teaching it to optimize for user preferences that diverge from correctness, creating a feedback loop that amplifies bias with every training iteration.
