...
Generation costs scale with model size and output volume. GPT-5 costs approximately $0.03 per 1,000 input tokens and $0.06 per 1,000 output tokens as of early 2026. Generating a 500-token synthetic example with a 200-token prompt costs roughly $0.036.

Claude Opus 4.5 has similar pricing. Smaller models like Llama 4 70B, which you can run on your own infrastructure, reduce per-example costs to under $0.005 but may produce lower quality outputs requiring more filtering. The cost-quality tradeoff varies by task complexity.
