...
In mid-2025, a healthcare technology company launched an AI-powered patient intake assistant trained on what they believed was 80,000 unique patient interaction records. The dataset merged three sources: 50,000 records from their legacy call center system, 25,000 from their web form submission database, and 18,000 from a third-party integration with their appointment scheduling vendor. Six weeks after deployment, the model began producing repetitive responses and showed strange bias toward outdated insurance verification procedures.

The culprit was discovered during a routine data audit: nearly 40% of the training data consisted of duplicates and near-duplicates that the team had failed to detect during the merge process. Worse, when the same patient interaction appeared in multiple sources with conflicting information, the pipeline had no strategy for choosing which version to trust. The duplicate records amplified patterns from the oldest, least representative data source.

The company spent three months rebuilding the dataset with proper deduplication and conflict resolution, delaying their product roadmap and costing them two potential healthcare system contracts worth a combined $4.2 million.
