...
In March 2025, a legal technology company discovered that 18% of their contract clause annotations were systematically incorrect. The pattern was subtle but devastating: annotators were consistently mislabeling jurisdiction clauses that appeared in the bottom third of document pages. The problem wasn't annotator competence or training quality. The labeling interface displayed documents in a fixed-height viewport with jurisdiction dropdown menus positioned at the top of the screen. When annotators scrolled down to read clauses near page bottoms, the dropdown menu remained anchored at the top. Each jurisdiction selection required scrolling back up, selecting from a 47-option dropdown, then scrolling back down to verify context. Within two hours of continuous work, annotators began selecting jurisdictions from memory rather than scrolling repeatedly. The cognitive overhead of the interface design itself was causing systematic label degradation. The company spent seven months and $340,000 retraining models on corrected labels. The interface had converted competent annotators into error-prone ones through pure friction.
