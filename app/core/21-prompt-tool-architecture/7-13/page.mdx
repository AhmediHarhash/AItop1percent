...
Multi-call composition is common. To get comprehensive customer data, your API requires calling /customers/{id}, /customers/{id}/orders, and /customers/{id}/preferences. Your tool get_customer_profile makes all three calls internally and returns unified results. The model makes one tool call, not three.
...
Flat structures beat nested structures. Instead of returning {"customer": {"name": {"first": "Alice", "last": "Smith"}}}, return {"customer_first_name": "Alice", "customer_last_name": "Smith"}. Flatter data is easier for models to extract and cite.

Labeled values beat unlabeled values. Instead of returning {"orders": [12345, 12346, 12347]}, return {"order_numbers": [12345, 12346, 12347], "order_count": 3}. Labels make data interpretation obvious. Including metadata like counts helps models understand what they're looking at.
...
Error responses need structure too. Don't return plain text errors like "Order not found". Return {"status": "error", "error_code": "NOT_FOUND", "error_message": "Order #12345 does not exist", "suggestion": "Check the order number and try again"}. Structured errors enable models to handle failures intelligently.
