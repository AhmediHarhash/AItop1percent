...
For binary metrics like accuracy or conversion rate, use two-proportion z-tests. Variant A got 920 successes out of 1,000 trials (92.0%). Variant B got 950 successes out of 1,000 trials (95.0%). The z-test yields p less than 0.001, strongly significant. You conclude variant B is better.
...
The basic pattern is: generate a random number between 0 and 1 for each request or user, if the number is less than the rollout percentage, use variant B, otherwise use variant A. This implements probabilistic traffic splitting.
...
The healthcare company requires three criteria for promotion: statistically significant improvement at p less than 0.01, effect size of at least 2 percentage points, and no segment showing degradation greater than 1 point. All three must be met.
