...
Prefilling is more reliable than instruction for format control. You can tell the model "respond in JSON" in the system prompt, but it might still add explanatory text before the JSON. If you prefill with an opening brace character, it must complete valid JSON from that starting point. This is especially important for programmatic consumption where parsing failures break pipelines.
...
Assistant prefilling also works for partial generations. You can prefill multiple steps: "Step 1: [completed step text]. Step 2:" and the model continues from Step 2. This lets you resume interrupted generations or guide multi-step reasoning. The model sees the partial work and completes the remaining steps.
