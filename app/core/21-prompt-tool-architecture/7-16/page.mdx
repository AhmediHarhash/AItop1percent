...
Define latency budgets by user experience tier. Interactive queries need {"<"}300ms tool latency. Conversational queries tolerate {"<"}1s. Background operations accept {"<"}5s. Assign every tool call to a tier and enforce the budget.

Set Service Level Objectives for tool performance. "95% of search tool calls complete in {"<"}500ms." "99% of database tool calls complete in {"<"}1s." SLOs create accountability and guide optimization efforts. If you're not meeting SLOs, you know which tools need work.
