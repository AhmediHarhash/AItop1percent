...
For example, extracting clauses from a contract might produce XML like: {"<"}clause id="5.2" type="liability"{">"} Party A shall not be liable for {"<"}emphasis{">"} indirect damages{"<"}/emphasis{">"} exceeding {"<"}amount currency="USD"{">"}100000{"<"}/amount{">"}.{"<"}/clause{">"}. This preserves structure, annotations, and semantic markup in a way that JSON (which would need to represent this as nested objects with type annotations) cannot do as cleanly.
...
**Explicit format specification** in prompts is critical. "Generate XML output" is too vague. Specify the root element, required elements, attribute usage, and namespace requirements: "Generate an XML document with root element {"<"}report{">"}, containing {"<"}company{">"},  {"<"}period{">"}, and {"<"}financials{">"} elements. Use attributes for IDs and metadata. Ensure all tags are properly closed."
