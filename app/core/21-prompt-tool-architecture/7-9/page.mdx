...
Some systems use XML or JSON structures to delimit tool results visibly. The tool wraps its output in structured tags — a tool_result element with attributes for the tool name and status, enclosing the actual data, then a closing tag. This creates a clear boundary between tool output and other content. Models with strong structured data training recognize these delimiters and treat enclosed content as factual.
