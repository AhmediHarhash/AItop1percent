...
The token economics depend on image resolution and model pricing. Claude 3.5 Sonnet charges based on input tokens where images are converted to token counts based on pixel dimensions. GPT-4o uses a similar approach. A typical 8.5x11 page at 150 DPI contains approximately 1.6 million pixels. At current pricing, this translates to significant per-page costs when processing thousands of documents.
