...
Streaming structured output is technically challenging because partial JSON is usually invalid. You can't parse "{"name": "John" until the model generates the closing brace. If you try to render partial data, you don't know which fields are complete and which are still being generated.
