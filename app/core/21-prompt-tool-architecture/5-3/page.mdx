...
Temperature is the primary source of flakiness. Set temperature to 0 for test execution to maximize determinism. Even at temperature 0, some models show occasional variation, but it is minimal. If your production prompt uses temperature {">"} 0, test at temperature 0 for regression detection and run a smaller set of smoke tests at production temperature.
...
Multiple sampling reduces flakiness when temperature {">"} 0 is necessary. Run each test 3-5 times and aggregate results. If 4 out of 5 runs pass, mark the test as passed. If 0 or 1 out of 5 pass, mark as failed. If 2 or 3 out of 5 pass, mark as flaky and investigate. This provides statistical confidence.
...
Track flaky test rates as a metric. If 20% of your tests are flaky, your test suite is not providing value. Aim for {"\u003C"}1% flaky rate. This requires investment in deterministic testing infrastructure and assertion design.
