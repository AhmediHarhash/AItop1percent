...
Priority hierarchies help when you legitimately need multiple constraints. "Prioritize accuracy over brevity. If comprehensive explanation requires more than 100 words, exceed the limit." This structure acknowledges the trade-off and gives the model clear decision criteria. But use hierarchies sparingly. Most constraint conflicts indicate unclear requirements, not legitimate trade-offs.

Constraint conflicts often emerge during prompt evolution. The original prompt said "be concise." Six weeks later, someone added "include comprehensive details" to fix an underspecification problem. No one noticed the conflict. Regular prompt audits catch these accumulations before they degrade performance.
...
Role-playing instructions are particularly prone to cargo-cult adoption. "You are a helpful assistant" appears in countless prompts because it appeared in early GPT-3 examples. For most tasks, it adds nothing. The model doesn't need to roleplay helpfulness to perform classification or extraction. Delete role instructions unless you can measure their impact on output quality.

Chain-of-thought patterns have value for reasoning tasks but get cargo-culted into everything. If your task doesn't involve multi-step reasoning, logical inference, or verification, chain-of-thought adds latency and token cost without benefit. Save it for tasks where showing work actually improves results.
...
Template inheritance is the right way to reuse prompt components. Extract genuinely reusable pieces—output format specifications, common instructions, shared examples—into documented components. Compose new prompts from these components plus task-specific elements. This approach gives you reuse without cargo-cult inheritance of inappropriate context.

Version control helps track copy-paste evolution. When you see a prompt that's a modified copy of another, the diff shows you what changed. If 90% of the prompt is unchanged from the source but shouldn't be, that's copy-paste evolution. The diff makes it visible where fresh code would hide the problem.
...
Prompt drift often manifests as increasing token count without increasing performance. As people patch the prompt, they add instructions but rarely remove obsolete ones. If your prompt grows from 800 tokens to 1,900 tokens over six months but accuracy stays flat, you've accumulated drift. Regular compression reviews catch drift-induced bloat.

Change logs should accompany prompt modifications. When you modify a prompt, document what changed, why, and what the measured impact was. Future maintainers read the change log to understand evolution and avoid repeating failed experiments. Logs prevent drift by making intentional changes visible.
...
Tone misalignment is particularly common. Instructions might say "use professional business language" while examples use casual phrasing. The model sees the contradiction and outputs inconsistently. Audit tone across instructions and examples. They should demonstrate the same voice and formality level.

Format misalignment breaks output parsing. Your instructions specify JSON with specific field names. Your examples use different field names or a slightly different structure. The model follows the examples, and downstream parsing fails. Examples must exactly match specified formats, character for character.
...
Speculative edge case handling often gets the priorities wrong. You add handling for rare cases that never occur while missing common edge cases you didn't anticipate. Production data reveals actual edge case distribution. Build handling for the top 80% of observed edge cases, not the long tail of imagined ones.

When you do add edge case handling, add it with measurement. Deploy the handling, monitor how often it triggers, and validate that it produces correct behavior. Edge case logic that never triggers is dead code. Edge case logic that triggers but produces wrong results is worse than no handling at all.
...
Feature creep drives bloat. Every new requirement adds instructions. Few requirements trigger instruction removal. Combat feature creep by questioning new additions. Does this feature belong in this prompt. Could it be handled better elsewhere. Does adding this feature compromise core functionality. Not every feature request deserves prompt complexity.

Bloat indicators include: token count growing over time, decreasing accuracy despite more instructions, increasing modification difficulty, and engineers avoiding prompt changes because "it's too complex to modify safely." These symptoms suggest bloat has reached the point where refactoring is necessary, not optional.
...
Review should also check token efficiency. Is this prompt using tokens effectively. Are there obvious compression opportunities. Does token count align with task complexity. A 2,000-token prompt for simple classification should raise questions. Either the task is more complex than it appears, or the prompt has bloat.

Security review matters for prompts handling user input. Does this prompt properly delimit user-provided content. Are there injection risks. Does it handle adversarial inputs safely. Prompt security is often overlooked in code review. Make it explicit.
...
Simplify complex conditional logic. If your prompt has nested conditions ("if A then B, unless C, in which case D, except when E..."), the model will struggle. Flatten conditions. Make each case explicit. Clarity beats cleverness in prompt design.

Replace vague instructions with specific ones. "Be thorough" becomes "include: X, Y, and Z." "Use good judgment" becomes specific criteria for what constitutes good judgment in your context. Specificity produces consistency.
...
Refactoring reveals testing gaps. When you refactor and tests don't catch regressions, you've found missing test coverage. Add tests for the gaps. Refactoring and testing improve together through iteration.
...
Training matters. Teach your team to recognize anti-patterns. Show examples of good and bad prompts. Explain why certain patterns fail. Make anti-pattern knowledge explicit rather than expecting engineers to discover it through trial and error.

Metrics drive culture. If you measure and reward prompt quality—low token count with high accuracy, good test coverage, clean refactoring—your team will prioritize quality. If you only measure feature velocity, quality will suffer. Choose metrics that align with your quality goals.
