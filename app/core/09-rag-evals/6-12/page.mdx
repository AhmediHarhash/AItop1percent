# 6.12 — Evaluation Frameworks: RAGAS, DeepEval, and Custom Harnesses

In November 2025, a Series A healthcare startup spent six weeks building a custom RAG evaluation framework from scratch. They implemented retrieval precision and recall, answer correctness scoring using embedding similarity, faithfulness checking through entailment models, and a sophisticated relevance metric that weighted different aspects of answer quality. The framework worked beautifully on their internal test sets, producing clean numeric scores that tracked system improvements across iterations.

Then they attended an ML conference and learned about RAGAS, an open-source evaluation framework that implemented all the same metrics they had just built, plus several they had not considered, with better implementations and extensive testing on diverse datasets. Their custom framework had subtle bugs in metric calculations that inflated scores, lacked proper normalization for comparison across different answer lengths, and had no support for multi-turn conversations or context-dependent queries.

They had reinvented a wheel that already existed in better form, spending six engineering weeks they could have invested in actually improving their RAG system. The lesson was bitter but clear: understand what evaluation frameworks already provide before building custom infrastructure. The engineer who led the custom framework effort later admitted to the CEO that they had been unaware RAGAS existed because they never thought to search for existing RAG evaluation tools. They assumed evaluation was so domain-specific that no general framework could possibly work.
