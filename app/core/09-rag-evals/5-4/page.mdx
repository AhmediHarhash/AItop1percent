# 5.4 — Citation and Attribution: Teaching Models to Reference Sources

In July 2025, a legal technology company nearly lost a major law firm client when their case law research assistant provided an accurate summary of a precedent but cited the wrong case number. The summary was correct—it matched the content of the retrieved document—but the citation pointed to a different case entirely, one that had nothing to do with the query. The attorney used the citation to pull the full case file, realized the discrepancy, and immediately questioned the reliability of every other answer the system had ever provided. The company spent two months rebuilding trust, implementing citation verification systems, and retraining their models to generate accurate source references. The incident highlighted a critical truth: in high-stakes domains, users need to verify answers, and verification requires correct citations. No citation means no trust. Wrong citations are worse than no citations.
