# 6.8 — LLM-as-Judge for RAG: Designing Evaluation Prompts

In September 2024, a content moderation platform deployed an LLM judge to evaluate whether RAG-generated summaries were faithful to source documents. The judge was supposed to score faithfulness on a scale from zero to one. After two weeks in production, the team noticed that 94% of summaries received scores above 0.9, even though manual spot checks revealed significant faithfulness issues. The problem was the evaluation prompt. It asked "Is this summary faithful?" without defining faithfulness, providing examples, or specifying what to check. The judge defaulted to lenient scoring, approving summaries that sounded plausible. When the team redesigned the prompt with clear definitions, explicit criteria, and examples of faithful and unfaithful summaries, average scores dropped to 0.68, aligning with human judgment. The initial month of evaluation data was useless. The company had to re-evaluate thousands of summaries, delaying a product launch by six weeks. They learned that LLM judges are powerful but require careful prompt design to be reliable.
