...
In March 2025, a customer support automation platform discovered that their RAG system was answering user questions with a mix of information from their knowledge base and general knowledge from the model's training data. When a user asked about a specific product feature, the system would correctly cite documentation for some details but then add plausible-sounding but incorrect information about related features that were not in the docs. The issue was not retrieval failure; the right documents were being retrieved. It was that their system prompt simply said "Answer the user's question using the provided context," without explicitly instructing the model to restrict its response to only what the context supported. After they revised the prompt to say "Answer based only on the provided context; do not use outside knowledge," hallucination rates dropped by 64 percent. The problem had been costing them an estimated fifteen hours per week in support tickets from users who received incorrect information.
