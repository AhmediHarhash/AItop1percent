...
In July 2025, a biomedical research company built a RAG system for scientific literature search using standard single-vector embeddings. They embedded 500,000 research papers with a state-of-the-art model and achieved 88 percent recall at ten on their evaluation set. The system worked, but users complained that it missed papers with specific technical terms, struggled with rare acronyms, and failed to distinguish between subtly different concepts. The team investigated and discovered that their single-vector embeddings were compressing entire paragraphs into 768 dimensions, which inevitably lost fine-grained detail. They experimented with ColBERT, a multi-vector model that stores embeddings for every token and computes similarity with late interaction. Recall jumped to 94 percent, and users reported that search finally felt "precise." The tradeoff was that storage costs tripled—they now stored 100 to 200 vectors per document instead of one—and query latency increased from 40 milliseconds to 120 milliseconds. After running a cost-benefit analysis, they decided the quality improvement justified the cost and deployed ColBERT to production.
