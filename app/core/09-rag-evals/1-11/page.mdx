...
In July 2024, a startup built a general-purpose RAG platform marketed to every vertical. The architecture was elegant: chunk documents at 512 tokens, embed with state-of-the-art models, retrieve top-10, generate answers. Their first customer was a law firm that needed exact citations and jurisdiction filtering. The second was an e-commerce company needing real-time product availability. The third was a developer tools company needing code-aware chunking. All three churned within 90 days. The one-size-fits-all architecture fit nobody. By March 2025, the company had pivoted to vertical-specific RAG, with completely different chunking, retrieval, and generation strategies for legal, e-commerce, and code search. Revenue tripled, but they had to rebuild everything three times.
