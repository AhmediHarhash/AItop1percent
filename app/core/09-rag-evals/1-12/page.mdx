...
In November 2024, a SaaS company launched an AI-powered documentation assistant for their 50,000 users. They budgeted based on LLM inference costs: 2 million queries per month times 1000 output tokens at standard pricing. Three weeks into production, their monthly bill hit 180,000 dollars—six times the projection. The breakdown was brutal: embedding costs consumed 45 percent of the budget, vector database compute another 30 percent, LLM input tokens from RAG context 20 percent, and output generation only 5 percent. They had optimized the smallest cost component and ignored the largest ones. By January 2025, they had rebuilt their architecture around cost: aggressive caching, query deduplication, smaller embedding models, and context size limits. The bill dropped to 42,000 dollars monthly with minimal quality degradation.
