# 2.5 — Metadata Extraction and Enrichment for Chunks

A seven-figure legal malpractice settlement resulted from retrieval logic that treated all chunks as equally accessible. Opposing counsel's privileged documents lived in the same vector database as the client's documents. Semantic similarity worked beautifully—queries matched relevant chunks regardless of which side they belonged to—and that was the problem. An attorney cited a document during deposition that should have been invisible. The case was thrown out, the client sued, and the platform settled for millions. The root cause was not a traditional security breach. It was the absence of metadata for access control and the naive assumption that vector search would respect permissions it never knew existed.
