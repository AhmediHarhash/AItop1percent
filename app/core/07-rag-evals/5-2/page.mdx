# 5.2 — Context Relevance Filtering: Dropping Low-Quality Retrievals

More context is not always better. Sometimes it is worse. Adding low-relevance retrieved chunks to your prompt dilutes high-quality information, introduces noise, and gives the language model more opportunities to latch onto irrelevant details. The tenth-ranked chunk in your retrieval results is almost always worse than the fifth, and including it might hurt your answer quality more than it helps. Knowing when to stop retrieving and when to filter out marginal context is as important as knowing what to retrieve in the first place.
