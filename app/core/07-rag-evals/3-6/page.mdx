...
In March 2025, a legal research company decided to upgrade their embedding model from OpenAI's ada-002 to the newer text-embedding-3-large. They had embedded 15 million legal documents with ada-002 over the previous two years. The new model promised better performance, longer context windows, and improved domain understanding. They updated their query embedding pipeline to use the new model and deployed it to production. Within hours, retrieval quality collapsed. Users reported that search results were "completely broken" and "returned random documents." The engineering team investigated and realized they had created a catastrophic mismatch: they were embedding new queries with text-embedding-3-large but searching against a database of documents embedded with ada-002. The two models produce vectors in different semantic spaces, and cosine similarity between them is meaningless. They had to roll back the query pipeline, re-embed all 15 million documents with the new model—a process that took three weeks and cost 18,000 dollars in API fees—and coordinate a synchronized cutover. The migration wiped out a quarter of their engineering budget for the year.
