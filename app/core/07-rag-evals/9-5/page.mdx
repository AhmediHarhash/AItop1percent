# 9.5 — RAG Over Code: Searching and Understanding Codebases

In August 2024, a software company launched an internal AI coding assistant that used RAG over their monorepo containing six million lines of code across forty-seven microservices. They were ambitious and thorough. They chunked their entire codebase using a sliding window of two hundred lines, embedded everything with a state-of-the-art code embedding model, and built a vector index. They invested in infrastructure, optimized retrieval performance, and rolled it out to their engineering team with confidence. Engineers started asking questions like "How does the authentication service validate JWT tokens?" and "What database tables are modified by the order processing pipeline?"
