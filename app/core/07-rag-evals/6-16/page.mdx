# 6.16 — Judge Reliability for RAG: Drift, Bias, Calibration, and Disagreement

In December 2025, a Series C healthcare company discovered their RAG evaluation metrics had shifted dramatically despite no changes to their system. Over a three-week period in early December, their faithfulness scores dropped from ninety-one percent to eighty-four percent, answer relevance declined from eighty-eight percent to eighty-one percent, and several other LLM-based judge metrics showed similar degradations.

The engineering team scrambled to identify what they had broken. They reviewed every code change from the past month, examined retrieval indices for corruption, checked for embedding model updates, analyzed whether their document corpus had been modified, and investigated API dependencies. They found nothing. No component had changed. The system was producing identical outputs to queries it had answered a month earlier, yet evaluation scores had dropped seven points.

Then a senior engineer realized the timing coincided with OpenAI's release of a GPT-4 model update announced in late November. Their evaluation pipeline used GPT-4 as a judge for multiple metrics, requesting the model via the generic "gpt-4" identifier rather than pinning to a specific version. The model update had shifted judge behavior without their knowledge or consent.

The new model version was more critical in its assessments, producing lower scores for the same system outputs that the previous version had rated highly. Their system had not regressed; the measurement instrument had changed underneath them. They spent a week recalibrating their evaluation infrastructure, re-establishing baselines on the new judge version, and explaining to management why the metrics dropped despite no actual quality degradation.

The incident highlighted a fundamental challenge with LLM-based evaluation: when your measurement tool is a complex model that changes over time, measurement stability becomes a serious engineering problem that requires active management.
...
Open-source models as judges offer more control over drift. You can freeze a specific model version, host it yourself, and guarantee that the exact weights remain unchanged indefinitely. Models like Llama 3, Mistral, or specialized open-source judge models provide this stability.

The tradeoff is that open-source models may have lower judge quality than frontier closed models. A GPT-4 judge might correlate better with human judgments than a Llama 3 judge, producing more accurate assessments of answer quality. But the GPT-4 judge introduces drift risk that Llama 3 does not.
