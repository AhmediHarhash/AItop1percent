# 5.5 — Faithfulness Enforcement: Preventing Generation Beyond Context

**Faithfulness is binary: every claim in the answer is either supported by retrieved context or it is not.** There is no middle ground. A language model that adds "best practices" not found in your documents, that fills in gaps with general knowledge, or that makes reasonable inferences beyond what the text states is not being helpful. It is being unfaithful. In high-stakes domains, unfaithfulness is indistinguishable from hallucination, and hallucination destroys trust. Faithfulness enforcement means building systems that generate only what the context explicitly supports, nothing more.
