# 9.8 — Conversational RAG: Multi-Turn Retrieval Strategies

In March 2025, a customer support chatbot was deployed using a standard RAG architecture. Each user message triggered retrieval based solely on that message, independent of previous conversation history. A customer started a conversation: "I'm having trouble with my account." The bot retrieved general account troubleshooting articles and asked clarifying questions. The customer replied, "It's the password reset feature." The bot, treating this as a standalone query, retrieved articles about password reset features in general. The customer, frustrated, said, "It keeps saying my email isn't recognized." The bot retrieved articles about email recognition errors, but these articles were generic because the bot had lost the context that this was specifically about password reset, which was specifically about account issues. The customer gave up. The conversation log showed a coherent thread from the customer's perspective, but the bot treated each message as an isolated query. The support team reviewing the failed conversation realized the problem: conversations have context, history, and coreferences that single-turn RAG completely ignores.
