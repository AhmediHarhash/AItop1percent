# 7.9 — A/B Testing RAG Changes in Production

In July 2025, an e-commerce company's search team optimized their RAG-based product recommendation system by switching to a newer embedding model that scored 4 percentage points higher on standard benchmarks. They deployed the change to all users. Three days later, conversion rate had dropped 2.1 percent. Support tickets about "irrelevant recommendations" increased 40 percent. They rolled back the change, but the damage was done: lost revenue, user frustration, and team demoralization.
