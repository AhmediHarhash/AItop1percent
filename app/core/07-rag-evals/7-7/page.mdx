# 7.7 — Data Freshness Testing: Do Answers Reflect Current Documents

**Data freshness is not a feature, it is a correctness requirement.** When your RAG system serves answers based on documents that were updated weeks or months ago, it is not providing helpful information, it is confidently distributing misinformation. The system appears to work perfectly while users make decisions based on outdated policies, incorrect specifications, and superseded regulations. Testing data freshness means measuring time-to-propagation and detecting staleness before it causes harm.
