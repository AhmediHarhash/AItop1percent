# 5.6 — Answer Synthesis from Multiple Sources

Why does your RAG system retrieve five relevant chunks if it only uses information from the first one? Multi-source synthesis is the capability that turns retrieved chunks from independent documents into a coherent narrative. It identifies where sources agree, where they conflict, and where they complement each other. It aggregates compatible facts, surfaces contradictions, and explains discrepancies. Without synthesis, your system is a document fetcher that dumps context on users and makes them figure out how the pieces fit together. With synthesis, your system is an analyst that does the integration work users expect.
