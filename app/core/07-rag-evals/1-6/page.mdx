...
The CEO promises one-second latency because ChatGPT feels instant. Product commits to the target without asking engineering. Engineering builds a high-quality pipeline with all the components necessary for reliable answers: preprocessing, retrieval, reranking, generation, validation. The median latency is 2,800 milliseconds. The CEO is furious, users churn, and everyone blames engineering for missing the target. The real failure happened three months earlier when product agreed to a latency budget that was architecturally impossible without cutting quality.
