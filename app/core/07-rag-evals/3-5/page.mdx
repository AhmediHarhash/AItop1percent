...
Users complain that search results are "random" and "don't make sense" because the vector database uses Euclidean distance on normalized embeddings, producing nonsensical rankings. The team copied a configuration from a tutorial without understanding what the similarity metric meant or whether it matched their embedding model. OpenAI embeddings are L2-normalized and require cosine similarity. Euclidean distance is mathematically related but the database implementation applied it incorrectly. Switching to the correct metric fixed retrieval quality immediately. The lesson: similarity metrics are not interchangeable parameters you can ignore. They are mathematical assumptions that break your system when violated.
