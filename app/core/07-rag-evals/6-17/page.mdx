# 6.17 — Claim-Level Grounding Evaluation: Mapping Each Claim to a Source Span

**Answer-level faithfulness metrics hide partial hallucinations.** A financial services company reported ninety-three percent faithfulness to investors. Answers were mostly grounded in context. Then a regulatory audit revealed answers frequently mixed accurate facts with subtle errors or unsupported claims. Investment fee answers correctly stated management fees while misstating expense ratios. Eligibility answers correctly cited age requirements while hallucinating exceptions. Answer-level metrics checked if the overall answer was consistent with documents. Most claims were correct, so metrics looked good. But minority unsupported claims were invisible. A ninety-three percent faithful answer might have seven correct claims and one incorrect claim. That one incorrect claim might be the one that matters most. Claim-level evaluation decomposes answers into individual claims and verifies each separately. The new metric revealed only seventy-eight percent of answers had all claims grounded. The fifteen-point gap represented systematic failures invisible to answer-level metrics.
