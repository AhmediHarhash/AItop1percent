# 2.7 — Document Hierarchy: Preserving Structure Through Chunking

**Documents have structure—chapters, sections, subsections—and naive chunking destroys all of it.** Users query for VPN configuration and retrieve step 4 of a 10-step procedure with no indication that nine other steps exist. Security policy searches return a subsection titled "Exceptions" with no reference to the parent policy or what the exceptions apply to. Every chunk is treated as an independent, self-contained unit, which would be fine if documents were written that way. They are not. Hierarchical structure gives chunks meaning, and systems that discard it produce retrieval results that confuse users even when technically correct.
