# 7.5 — Edge Case Testing: Empty Retrievals, Contradictions, Stale Data

Across enterprise deployments, roughly two-thirds of production RAG systems exhibit hallucination rates exceeding 8 percent when retrieval returns no relevant results. These are not random errors. They are predictable failure modes that occur whenever your system encounters empty retrievals, contradictory documents, or stale data. The question is not whether your RAG system will face these edge cases, but when, and whether you tested for them before they caused damage.
