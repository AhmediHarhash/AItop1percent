...
Dense embeddings understand that "privacy violations" and "surveillance privacy breaches" are semantically related. They fail catastrophically when lawyers search for "Section 230 of the Communications Decency Act" and need exact statutory citations, not conceptually similar cases. Meanwhile, keyword search handles exact matches perfectly but misses paraphrases and conceptual queries. The production answer in 2026 is not choosing the better strategy. It is running both strategies in parallel and fusing results intelligently, because real users issue both semantic and exact-match queries in the same session.
