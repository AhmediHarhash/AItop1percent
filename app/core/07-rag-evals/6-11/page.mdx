# 6.11 — Automated RAG Evaluation Pipelines

In February 2026, a Series C fintech company discovered that their RAG system had been returning incorrect interest rate calculations for three weeks. The error affected approximately twelve hundred customer queries, resulted in two formal complaints to regulators, and required a painful audit of all affected customer interactions. The engineering team was baffled. They had a comprehensive evaluation suite with two hundred and forty test cases covering interest rate calculations, payment schedules, and amortization scenarios. Every test had passed in their staging environment before deployment.

The post-mortem revealed the root cause: their evaluation suite ran manually when an engineer remembered to run it, which happened approximately twice per month. The deployment that introduced the bug had bypassed evaluation entirely because the engineer was rushing to meet a deadline and figured the change was small enough to skip testing. Three lines of prompt modifications had subtly altered how the system parsed rate specifications, causing systematic errors on a specific query pattern.

An automated evaluation pipeline would have caught the issue in CI before merge, blocking the pull request until the regression was fixed. But they had prioritized feature development over evaluation infrastructure, treating evaluation as something that could be run occasionally rather than continuously. The cost of that prioritization was now being calculated in legal fees and lost customer trust. The CFO pointedly asked during the post-mortem why they had spent six months building evaluation tests that apparently did not actually prevent bugs from reaching production.
